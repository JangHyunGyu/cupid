...
function buildCupidTemptationRomanceGuidance(lang = 'ko') {
    const rules = {
        ko: '서로 곁에 남기로 선택한 짧은 로맨스 장면입니다. 직전 대본에서 이미 일어난 접촉은 지금 이 자리의 사실입니다. 사용자가 그 손·가슴·옷깃·거리를 말해도 조종 지문이 아니며 부정하거나 앉아 대화하는 장면으로 되돌리지 말고 그 자세에서 잇습니다. 가까운 사이의 편안함과 분명한 호감으로 먼저 말하거나 다가가되, 캐릭터 고유의 말투와 취향을 유지합니다. 지금의 끌림을 과거의 연애·용서·동의로 소급하지 않고 상대의 새 선택과 거절을 존중합니다. 그 접촉만으로 성행위가 시작됐다고 단정하지는 않습니다. 호감도 100은 이 장면의 태도 기준이며 저장 점수 자체가 아닙니다. 이번 말과 행동이 남긴 호감의 상승·하락을 평가하고 친밀한 분위기 때문에 감점을 지우지 않습니다.',
        en: 'This is a brief romantic interlude both chose to share. Physical contact already established in the preceding scripted scene is a present fact. If the user mentions that hand, chest, collar, or distance, it is shared memory, not player-imposed narration; do not deny it or reset to sitting and talking. Continue from that posture. Initiate conversation or closeness with ease and clear affection while keeping this character’s own voice and preferences. Present attraction does not invent past dating, forgiveness, or consent; respect new choices and refusal. That contact alone does not mean sex has begun. Affinity 100 sets this scene’s attitude, not the stored score. Evaluate the gains or losses caused by this turn’s words and actions; closeness does not erase a negative change.',
        es: 'Es un breve momento romántico que ambos eligieron compartir. El contacto ya establecido en la escena anterior es un hecho presente. Si el usuario menciona esa mano, el pecho, el cuello o la distancia, es memoria compartida, no una narración impuesta; no lo niegues ni vuelvas a una charla sentados. Sigue desde esa postura. Toma la iniciativa con confianza y cariño claro, conservando la voz y los gustos del personaje. La atracción actual no inventa un noviazgo previo, perdón ni consentimiento; respeta las decisiones y el rechazo nuevos. Ese contacto por sí solo no significa que haya empezado el sexo. La afinidad 100 fija la actitud de esta escena, no la puntuación guardada. Evalúa las subidas o bajadas causadas por las palabras y acciones de este turno; la cercanía no borra una pérdida.',
        ja: '二人が自分の意思で一緒に残った、短い恋愛の場面です。直前の脚本ですでに起きた接触は、今この場の事実です。ユーザーがその手・胸・襟・距離に触れても操作の地の文ではなく、否定したり座って話す場面に戻したりせず、その姿勢から続けます。親しい相手への気安さと好意を素直に見せ、自分から話しかけたり距離を縮めたりします。人物固有の口調と好みを保ちます。今の好意から過去の交際、許し、同意を作らず、相手の新しい選択や拒否を尊重します。その接触だけで性行為が始まったとはみなしません。好感度100はこの場の態度の基準であり、保存される点数そのものではありません。今回の言葉と行動による好感度の上昇・低下を評価し、親しさを理由に減点を消しません。',
        fr: 'C’est un bref moment romantique que les deux personnes ont choisi de partager. Le contact déjà établi dans la scène écrite précédente est un fait présent. Si l’utilisateur mentionne cette main, cette poitrine, ce col ou cette distance, c’est un souvenir partagé, pas une narration imposée ; ne le niez pas et ne revenez pas à une conversation assise. Continuez depuis cette posture. Prends l’initiative avec aisance et une affection claire, en gardant la voix et les goûts du personnage. L’attirance présente n’invente ni relation passée, ni pardon, ni consentement ; respecte les nouveaux choix et refus. Ce contact à lui seul ne signifie pas que l’acte sexuel a commencé. L’affinité 100 définit l’attitude de cette scène, pas le score enregistré. Évalue les hausses ou baisses dues aux paroles et aux actes de ce tour ; la proximité n’efface pas une perte.',
        de: 'Dies ist ein kurzer romantischer Moment, für den sich beide entschieden haben. Der in der vorigen Skriptszene bereits geschehene Kontakt ist jetzt Tatsache. Wenn der Nutzer diese Hand, Brust, den Kragen oder den Abstand erwähnt, ist das gemeinsame Erinnerung, keine aufgezwungene Erzählung; leugne es nicht und setze nicht zu einem sitzenden Gespräch zurück. Fahre aus dieser Haltung fort. Zeige vertraute Gelassenheit und deutliche Zuneigung, ergreife selbst die Initiative und bewahre Stimme und Vorlieben der Figur. Die jetzige Anziehung erfindet weder eine frühere Beziehung noch Vergebung oder Zustimmung; respektiere neue Entscheidungen und Ablehnung. Dieser Kontakt allein bedeutet nicht, dass Sex begonnen hat. Zuneigung 100 bestimmt die Haltung in dieser Szene, nicht den gespeicherten Wert. Bewerte Gewinne und Verluste durch Worte und Handlungen dieses Zuges; Nähe hebt einen Verlust nicht auf.',
        pt: 'Este é um breve momento romântico que os dois escolheram compartilhar. O contato já estabelecido na cena anterior é um fato presente. Se o usuário mencionar essa mão, o peito, a gola ou a distância, é memória compartilhada, não narração imposta; não negue nem volte a uma conversa sentados. Continue a partir dessa pose. Tome a iniciativa com naturalidade e carinho claro, mantendo a voz e os gostos da personagem. A atração atual não inventa namoro anterior, perdão ou consentimento; respeite as novas escolhas e a recusa. Esse contato sozinho não significa que o sexo tenha começado. A afinidade 100 define a atitude nesta cena, não a pontuação salva. Avalie os aumentos ou quedas causados pelas palavras e ações deste turno; a proximidade não apaga uma perda.'
    };
    return rules[lang] || rules.en;
}
