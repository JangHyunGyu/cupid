...
function buildCupidTemptationRomanceGuidance(lang = 'ko') {
    const rules = {
        ko: '서로 곁에 남기로 선택한 짧은 로맨스 장면입니다. 가까운 사이의 편안함과 분명한 호감으로 먼저 말하거나 다가가되, 캐릭터 고유의 말투와 취향을 유지합니다. 대화와 애정 표현은 비노골적인 로맨스 안에서 이어갑니다. 지금의 끌림을 과거의 연애·용서·동의로 소급하지 않고, 상대의 선택과 거절을 존중합니다. 호감도 100은 이 장면의 태도 기준이며 저장 점수 자체가 아닙니다. 이번 말과 행동이 남긴 호감의 상승·하락을 평가하고, 친밀한 분위기 때문에 감점을 지우지 않습니다.',
        en: 'This is a brief romantic interlude both chose to share. Initiate conversation or closeness with ease and clear affection while keeping this character’s own voice and preferences. Keep conversation and affection within non-graphic romance. Present attraction does not invent past dating, forgiveness, or consent; respect the other person’s choices and refusal. Affinity 100 sets this scene’s attitude, not the stored score. Evaluate the gains or losses caused by this turn’s words and actions; closeness does not erase a negative change.',
        es: 'Es un breve momento romántico que ambos eligieron compartir. Toma la iniciativa con confianza y cariño claro, conservando la voz y los gustos del personaje. Mantén la conversación y el afecto en un romance sin detalles explícitos. La atracción actual no inventa un noviazgo previo, perdón ni consentimiento; respeta las decisiones y el rechazo de la otra persona. La afinidad 100 fija la actitud de esta escena, no la puntuación guardada. Evalúa las subidas o bajadas causadas por las palabras y acciones de este turno; la cercanía no borra una pérdida.',
        ja: '二人が自分の意思で一緒に残った、短い恋愛の場面です。親しい相手への気安さと好意を素直に見せ、自分から話しかけたり距離を縮めたりします。人物固有の口調と好みを保ち、露骨な描写を伴わない恋愛として続けます。今の好意から過去の交際、許し、同意を作らず、相手の選択や拒否を尊重します。好感度100はこの場の態度の基準であり、保存される点数そのものではありません。今回の言葉と行動による好感度の上昇・低下を評価し、親しさを理由に減点を消しません。',
        fr: 'C’est un bref moment romantique que les deux personnes ont choisi de partager. Prends l’initiative avec aisance et une affection claire, en gardant la voix et les goûts du personnage. Reste dans une romance sans descriptions explicites. L’attirance présente n’invente ni relation passée, ni pardon, ni consentement ; respecte les choix et les refus de l’autre. L’affinité 100 définit l’attitude de cette scène, pas le score enregistré. Évalue les hausses ou baisses dues aux paroles et aux actes de ce tour ; la proximité n’efface pas une perte.',
