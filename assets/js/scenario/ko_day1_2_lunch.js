/**
 * ============================================================================
 * CUPID - Day 1 Lunch Scenario (Korean Version)
 * ============================================================================
 * 
 *  파일 정보
 * ----------------------------------------------------------------------------
 * 파일명: ko_day1_2_lunch.js
 * 언어: 한국어 (Korean)
 * 일차: Day 1 (1일차)
 * 시간대: Lunch (점심)
 * 
 *  시나리오 개요
 * ----------------------------------------------------------------------------
 * 주인공이 전학 온 첫날 점심시간의 다양한 활동을 다룹니다.
 * 플레이어의 선택에 따라 여러 장소에서 각기 다른 캐릭터들과 교류할 수 있으며,
 * 이후 방과 후 활동 및 연락처 교환에 영향을 미치는 주요 분기점입니다.
 * 
 *  주요 이벤트
 * ----------------------------------------------------------------------------
 * 1. 서연과의 옥상 점심 - 도시락 데이트 및 첫 연락처 교환 시도
 * 2. 별관 도서관 (유나) - 신비로운 소녀 유나와의 첫 만남
 * 3. 체육관 (다인) - 활발한 다인과의 배구 연습 및 통성명
 * 4. 매점/보건실/교실 - 기타 캐릭터(선생님, 보건샘 등)와의 짧은 조우 및 휴식
 * 
 *  조건 플래그 설명 (Condition Flags)
 * ----------------------------------------------------------------------------
 * personality_study    : 자기소개 시 공부 위주 성격 선택 시 분기
 * personality_active   : 자기소개 시 활발한 성격 선택 시 분기
 * 
 *  설정 플래그 설명 (Set Flags)
 * ----------------------------------------------------------------------------
 * met_seoyeon          : 서연과 함께 시간을 보냈을 때 설정
 * ate_lunch_seoyeon    : 서연과 점심을 먹었을 때 설정
 * has_number_seyoun    : 서연의 연락처를 획득했을 때 설정
 * met_yuna             : 유나와 처음 만났을 때 설정
 * knows_name_yuna      : 유나의 이름을 알게 되었을 때 설정
 * met_dain             : 다인과 처음 만났을 때 설정
 * knows_name_dain      : 다인의 이름을 알게 되었을 때 설정
 * has_any_contact      : 첫 연락처를 획득했을 때 설정
 * 
 *  호감도 변화 (Stats Changes)
 * ----------------------------------------------------------------------------
 * 서연 (Seoyeon)        : 데이트 분위기 및 질문 답변에 따라 변화
 * 유나 (Yuna)           : 첫 대화 반응에 따라 변화
 * 기타 히로인          : 조우 시 소폭 변화 가능
 * 
 * ============================================================================
 */
...
Object.assign(SCENARIO[1], {
    ...
    "rooftop_1": {
        ...
        background: "assets/images/background/top_school.png",        character: "assets/images/characters/seyoun_normal.png",        setFlag: "ate_lunch_seoyeon",
        next: "rooftop_1_2"
    },
    "rooftop_1_2": {
        name: "서연",
