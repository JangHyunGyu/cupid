 ...
 *  ?Œì¼ ?•ë³´
 * ----------------------------------------------------------------------------
 * ?Œì¼ëª? en_day1_3_afterschool.js
 * ?¸ì–´: ?ì–´ (English)
 * ?¼ì°¨: Day 1 (1?¼ì°¨)
 * ?œê°„?€: After School (ë°©ê³¼ ??
 * 
 *  ?œë‚˜ë¦¬ì˜¤ ê°œìš”
 * ----------------------------------------------------------------------------
 * ?„í•™ ì²«ë‚  ?¼ê³¼ê°€ ?ë‚œ ?„ì˜ ?œë™???¤ë£¹?ˆë‹¤. ?™êµ ê³³ê³³???ìœ ë¡?²Œ ?ìƒ‰?˜ë©°
 * ?ˆë¡œ?¸ë“¤??ë°©ê³¼ ???œë™??ì°¸ì—¬?˜ê±°?? ?´ì„ ? ìƒ?˜ê³¼???ë‹´, ?¹ì? ë°”ë¡œ ê·€ê°€?˜ëŠ” 
 * ê²½ë¡œë¥?? íƒ?????ˆìŠµ?ˆë‹¤. ê°??¥ì†Œ?ì„œ???œë™?€ ?¸ê°?„ì? ?¥í›„ ?°ë½ì²??•ì¸ 
 * ?´ë²¤?¸ì— ???í–¥??ë¯¸ì¹©?ˆë‹¤.
 * 
 *  ì£¼ìš” ?´ë²¤??
 * ----------------------------------------------------------------------------
 * 1. ?¥ì†Œ ? íƒ - ?™ìƒ?Œì‹¤(?œì—°), ?„ì„œê´€(? ë‚˜), ì²´ìœ¡ê´€(?¤ì¸), ë³´ê±´??ë³´ê±´??, êµë¬´???´ì„??, ê·€ê°€
 * 2. ?œì—°???™ìƒ???…ë¬´ - ?œë¥˜ ?•ë¦¬ ?•ê¸°, ì¹´í˜ ?°ì´???œì•ˆ, ?°ë½ì²?êµí™˜
 * 3. ? ë‚˜???„ì„œê´€ ë³„ê? - ?™êµ??ì§„ì‹¤???€??ê²½ê³ , ? ë‚˜?€???•ì„œ??êµê°, ?°ë½ì²?êµí™˜
 * 4. ?¤ì¸??ë°°êµ¬ë¶€ ?°ìŠµ - ?‘ì› ?ëŠ” ?°ìŠµ ?•ê¸°(ì²´ë ¥ ì¤‘ì‹œ), ?¡ë³¶???°ì´?? ?°ë½ì²?êµí™˜
 * 5. ë³´ê±´ ? ìƒ???´ì„ ? ìƒ??- ?ë‹´ ë°??¬ì ???€?? ?°ë½ì²?êµí™˜ ê¸°íšŒ
 * 6. ?˜êµ ë°??¸ì˜??- ?¼ê³¼ ë§ˆë¬´ë¦?ë°??´ì¼???„í•œ ?´ì‹
 * 
 *  ì¡°ê±´ ?Œë˜ê·??¤ëª… (Condition Flags)
 * ----------------------------------------------------------------------------
 * met_*                : ?ì‹¬?œê°„???´ë‹¹ ?ˆë¡œ?¸ì„ ë§Œë‚¬?”ì? ?•ì¸ (êµ¬ë©´/ì´ˆë©´ ?€??ë¶„ê¸°)
 * has_number_*         : ?´ë? ?°ë½ì²˜ë? ê°€ì§€ê³??ˆëŠ”ì§€ ?•ì¸ (ì¤‘ë³µ êµí™˜ ë°©ì?)
 * 
 *  ?¤ì • ?Œë˜ê·??¤ëª… (Set Flags)
 * ----------------------------------------------------------------------------
 * met_*                : (ì´ˆë©´??ê²½ìš°) ?ˆë¡œ?¸ê³¼ ì²˜ìŒ ë§Œë‚¬?Œì„ ê¸°ë¡
 * knows_name_*         : ?ˆë¡œ?¸ì˜ ?´ë¦„???Œê²Œ ??
 * helped_seoyeon       : ?œì—°???…ë¬´ë¥??„ì?ì£¼ì—ˆ?Œì„ ê¸°ë¡
 * dated_*_day1         : ë°©ê³¼ ???¹ì • ?ˆë¡œ?¸ê³¼ ?°ì´??ì¹´í˜ ??ë¥??ˆìŒ??ê¸°ë¡
 * has_number_*         : ?¹ì • ?ˆë¡œ?¸ì˜ ?°ë½ì²˜ë? ?ë“??
 * has_any_contact      : ìµœì†Œ ??ëª??´ìƒ???°ë½ì²˜ë? ?•ë³´??
 * 
 *  ?¸ê°??ë³€??(Stats Changes)
 * ----------------------------------------------------------------------------
 * ëª¨ë“  ?ˆë¡œ?¸ë“¤???€??? íƒì§€???°ë¼ ?¸ê°???ìŠ¹/?˜ë½ ê°€??
 * 
 *  ?¹ì´ ?¬í•­
 * ----------------------------------------------------------------------------
 * - sunset: true - ë°©ê³¼ ???¸ì„ ë¶„ìœ„ê¸??°ì¶œ???„í•œ ?„ì—­ ë°°ê²½ ?ì„±
 * - excludeCondition: ?¹ì • ì¡°ê±´??ë§Œì¡±?˜ì? ?Šì„ ?Œë§Œ ?¤í–‰?˜ëŠ” ë¶„ê¸° ?œìš©
 * 
 * ============================================================================
 */
...
// Day 1 ?œë‚˜ë¦¬ì˜¤ ê·¸ë£¹ ì´ˆê¸°??
if (!SCENARIO[1]) {
    SCENARIO[1] = {};
}

/**
 * [Day 1 - After School] ???°ì´???•ì˜
 */
Object.assign(SCENARIO[1], {
    ...
    "after_seoyeon_cafe": {
        name: "Seoyeon",
        text: "(After finishing work, we stopped by a small cafÃ© in front of school.)",
