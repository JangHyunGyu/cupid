 ...
 *  시나리오 개요
 * ----------------------------------------------------------------------------
 * 전학 2일차 점심시간의 다양한 활동을 다룹니다. 본격적으로 캐릭터 간의 갈등
 * (서연 vs 다인)이 발생하며, 유나 루트를 통해 학교의 비밀에 한 걸음 다가가는 
 * 중요한 분기점입니다. 보건실 루트에서는 긴 수면을 통해 방과 후 시간대로 직접 
 * 이동하는 특수 연출이 포함되어 있습니다.
 * 
 *  주요 이벤트
 * ----------------------------------------------------------------------------
 * 1. 점심 장소 선택 - 서연(학생회실), 유나(도서관 뒷마당), 다인(체육관/매점), 보건실, 교무실
 * 2. 서연 루트 - 직접 만든 샌드위치 시식, 창밖의 다인과 조우하며 발생하는 묘한 질투심
 * 3. 유나 루트 - 쪽지 약속 이행, 학교의 비밀 언급, 구관 창고 방문 및 호감도 분기
 * 4. 다인 루트 - 매점에서의 간식 쟁탈전, 서연과의 우연한 만남 및 주인공을 둘러싼 기싸움
 * 5. 보건실 루트 - 두통으로 인한 휴식, 오후 수업 스킵, 노을 질 때까지의 긴 수면, 보건샘의 대담한 유혹
 * 
 *  조건 플래그 설명 (Condition Flags)
 * ----------------------------------------------------------------------------
 * met_*                : 해당 히로인을 이미 만났는지 여부 (초면/구면 대사 분기)
 * day2_ate_lunch_seoyeon: 서연과 방금 점심을 먹었는지 확인 (매점에서 서연 조우 시 활용)
 * has_number_*         : 연락처 보유 여부에 따른 연락처 교환 제안 분기
 * 
 *  설정 플래그 설명 (Set Flags)
 * ----------------------------------------------------------------------------
 * met_* / knows_name_* : 히로인과의 첫 만남 및 이름 획득 기록
 * day2_ate_lunch_seoyeon: 서연과 점심을 먹었음을 기록
 * visited_warehouse_at_lunch: 유나와 창고를 방문했음을 기록
 * day2_met_*_lunch     : 점심시간에 특정 히로인과 시간을 보냈음을 기록
 * invited_nurse_home   : 보건 선생님에게 집으로 초대받았음을 기록 (특수 이벤트)
 * 
 *  호감도 변화 (Stats Changes)
 * ----------------------------------------------------------------------------
 * 선택지에 따라 각 히로인과의 호감도가 크게 변동 (특히 삼자대면 상황 주의)
 * 
 *  특이 사항
 * ----------------------------------------------------------------------------
 * - 보건실 루트: `sunset: true` 속성을 통해 점심시간에서 바로 방과 후(노을)로 시간대 전환
 * - type: "free_talk": 유나, 다인, 보건 선생님과의 심도 깊은 대화를 위한 AI 모듈 활용
 * 
 * ============================================================================
 */
