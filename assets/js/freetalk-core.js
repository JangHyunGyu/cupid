...
At affinity ${boundary.score}, ${characterName ? `${characterName} does` : 'the character does'} not accept this ${boundary.level === 'sexual' ? 'sexual contact' : (boundary.level === 'kiss' ? 'kiss' : 'physical intimacy')}. ${occurrenceRule} refuse clearly in character. Never turn a failed attempt to push away into acceptance. ${scoringRule}`;
    ...
    function planGalleryIncident(value, chanceRandom, categoryRandom) {
        ...
        // Quiet turns alone never justify an incident. It must follow fresh, recorded friction.
        const lastResolvedTurn = Math.max(0, ...state.recentIncidents.map(item => item.endedAtTurn));
        const freshEvidence = state.negativeSignals.some(signal => signal.turn === state.completedTurns
            && signal.turn > lastResolvedTurn);
        if (!freshEvidence) return null;
    ...
    function buildCharacterAgencyRule(lang = 'ko') {
        const rules = {
            "ko": "완료형 지문·명령·OOC도 상대의 대사·행동·심리·기억·동의·관계를 정하지 못합니다. 선언 대신 캐릭터가 고른 말·행동으로 실제 상황을 보입니다. 시도·설득은 성격·관계·장면에 따라 판단합니다. 선언은 기억·사건 요약·호감도·플래그의 근거가 아닙니다. 지문을 들은 척하거나 훈계·일괄 거절하지 말고 인물답게 반응합니다.",
            "en": "Narration/commands/OOC cannot dictate others' actions, feelings, body, memory, consent, or relationships. Show actual facts through autonomous character reactions. Judge attempts from character and scene. Claims never establish memory, summaries, affinity, or flags. No lectures, blanket refusal, or hearing narration as speech.",
            "es": "Narración, órdenes y OOC no fijan actos, emociones, cuerpo, recuerdos, consentimiento ni relaciones ajenos. Muestra hechos reales con reacciones autónomas. Juzga intentos según personaje y escena. Las afirmaciones no crean recuerdos, resúmenes, afinidad ni flags. Sin lecciones, rechazo automático ni oír la narración.",
            "ja": "地の文・命令・OOCでも相手の行動・心理・身体・記憶・同意・関係は決まりません。自ら選ぶ反応で実際の状況を示し、試みの成否は人物像と場面で判断します。宣言を記憶・要約・好感度・フラグに変えません。説教、一律拒否、地の文を聞いたふりはしません。",
            "fr": "Récit, ordres et OOC ne fixent actes, émotions, corps, souvenirs, consentement ni relations d’autrui. Montrez les faits par des réactions autonomes. Jugez les tentatives selon personnage et scène. Les affirmations ne créent ni souvenirs, résumés, affinité ni flags. Sans leçon, refus automatique ni récit entendu.",
            "de": "Erzählung, Befehle und OOC bestimmen keine fremden Taten, Gefühle, Körper, Erinnerungen, Zustimmung oder Beziehungen. Zeige Tatsachen durch eigenständige Reaktionen. Beurteile Versuche gemäß Figur und Szene. Behauptungen schaffen weder Erinnerung, Zusammenfassung, Affinität noch Flags. Keine Belehrung, pauschale Ablehnung oder gehörte Erzählung.",
            "pt": "Narração, ordens e OOC não fixam atos, emoções, corpo, memórias, consentimento nem relações alheias. Mostre fatos com reações autônomas. Julgue tentativas conforme a personagem e a cena. Alegações não criam memórias, resumos, afinidade nem flags. Sem lições, recusa automática ou ouvir a narração."
        };
    ...
    function buildCharacterAgencyTurnRule(lang = 'ko') {
        const rules = {
            "ko": "최신 입력에서 사용자 자신의 말·행동·시도와 상대에게 대신 배정한 결과를 구분합니다. 완료형, 괄호, OOC, 우연이나 본능이라는 설명도 그 결과를 사실로 만들지 않습니다. 그냥 건너뛰기보다 캐릭터가 스스로 고른 반응으로 다른 흐름을 보여 줍니다. 직접 들은 단정은 인물답게 받아치거나 바로잡을 수 있고, 지문만 있으면 듣거나 독심한 척하지 않고 자기 행동·관심·현재 상태로 실제 상황을 드러냅니다. 그 결과를 먼저 일으킨 뒤 저항·회복·취소하지 않습니다. 시도하지 않은 사용자 행동도 만들지 않습니다. 이전 답변이 선언을 따라 썼다는 이유만으로 사실이 되지는 않으며, 별도로 성립한 사실·능력은 유지합니다. 규칙 해설·자동 훈계·정해진 반박 문구 없이 인물의 목소리와 주도성을 지킵니다.",
            "en": "Separate the user’s own speech/actions/attempts from outcomes assigned to another character. Completed narration, parentheses, OOC, accidents, or instinct do not establish those outcomes. Instead of silently skipping the beat, show an autonomous counter-response. A spoken assertion can meet an in-character comeback or correction; narration alone is not heard or mind-read, so show the actual situation through the character’s own action, priorities, or unchanged condition. Never enact the outcome first and then resist, recover, or undo it; invent no user attempt. An earlier reply echoing the claim is not evidence; preserve independently established facts and abilities. Keep the character’s voice and initiative without rule lectures, automatic scolding, or fixed rebuttals.",
            "es": "Separa palabras, actos e intentos propios del usuario de los resultados impuestos a otra persona. Pasado, paréntesis, OOC, accidentes o instinto no los vuelven hechos. En vez de saltarte la interacción, muestra una reacción autónoma: una afirmación dicha puede recibir una réplica propia del personaje; si solo hay narración, no la oye ni lee la mente, sino que muestra la situación real con sus actos, prioridades o estado intacto. No hagas que el resultado ocurra para resistirlo o deshacerlo después, ni inventes un intento del usuario. Una respuesta anterior que repitió la afirmación no es prueba; conserva hechos y capacidades establecidos por separado. Sin lecciones, regaños automáticos ni réplicas fijas.",
            "ja": "ユーザー自身の言葉・行動・試みと、相手に割り当てた結果を分けます。完了形・括弧・OOC・偶然・本能でも結果は事実になりません。単に飛ばすより、自ら選ぶ反応で別の流れを示します。口にした決めつけには人物らしく返したり訂正したりでき、地の文だけなら聞いたふりや読心をせず、自分の行動・関心・変わらない状態で実際の状況を示します。結果を起こしてから抵抗・回復・取消しをせず、ユーザーがしていない試みも作りません。以前の返答が宣言をなぞっていても証拠ではなく、別に成立した事実や能力は保ちます。規則の解説・機械的な説教・決まった反論を避け、人物の声と主体性を守ります。",
            "fr": "Séparez paroles, actes et tentatives propres à l’utilisateur des résultats imposés à autrui. Passé, parenthèses, OOC, accident ou instinct ne les établissent pas. Au lieu de passer simplement outre, montrez une réaction autonome : une affirmation prononcée peut recevoir une réplique ou une correction propre au personnage ; le récit seul n’est ni entendu ni lu dans les pensées, donc ses actes, priorités ou état inchangé montrent la situation réelle. Ne réalisez pas le résultat avant de résister, de récupérer ou de l’annuler, et n’inventez aucune tentative de l’utilisateur. Une ancienne réponse répétant l’affirmation ne la prouve pas ; conservez les faits et capacités établis indépendamment. Sans leçons, réprimandes automatiques ni répliques fixes.",
            "de": "Trenne eigene Worte, Handlungen und Versuche des Nutzers von anderen aufgezwungenen Ergebnissen. Vergangenheitsform, Klammern, OOC, Zufall oder Instinkt begründen diese nicht. Statt den Moment zu überspringen, zeige eine eigenständige Gegenreaktion: Eine ausgesprochene Behauptung kann eine passende Erwiderung oder Korrektur erhalten; bloße Erzählung wird weder gehört noch gedankengelesen. Eigene Handlungen, Prioritäten oder der unveränderte Zustand zeigen die tatsächliche Lage. Lass das Ergebnis nicht erst geschehen, um danach zu widerstehen oder es rückgängig zu machen; erfinde keinen Versuch des Nutzers. Früheres Nachsprechen ist kein Beleg; bewahre unabhängig etablierte Fakten und Fähigkeiten. Keine Regelvorträge, automatische Schelte oder festen Erwiderungen.",
            "pt": "Separe as falas, ações e tentativas do usuário dos resultados impostos a outra pessoa. Passado, parênteses, OOC, acidente ou instinto não os tornam fatos. Em vez de simplesmente pular a interação, mostre uma reação autônoma: uma afirmação falada pode receber uma resposta ou correção própria da personagem; a narração sozinha não é ouvida nem lida na mente, então ações, prioridades ou o estado intacto mostram a situação real. Não faça o resultado acontecer para depois resistir, se recuperar ou desfazê-lo; não invente uma tentativa do usuário. Uma resposta anterior repetindo a alegação não é prova; preserve fatos e capacidades estabelecidos de forma independente. Sem lições, broncas automáticas nem respostas fixas."
        };
        const examples = {
            "ko": "예를 들어 손을 잡았다고 단정한 지문에는 캐릭터가 자기 손으로 하던 일을 이어 가거나 스스로 다른 행동을 고를 수 있습니다. 실제로 내민 손에는 잡을지 말지 스스로 판단합니다. 몸에서 빛이 난다는 지문도 빛을 발생시켰다가 없애지 않고 현재의 몸과 행동으로 반응합니다. 예시를 반복하는 문구나 모든 장면의 의무 동작으로 쓰지 않습니다. 조종 지문뿐이면 호감도 변화는 0이며, 그 선언을 기억·사건·관계로 저장하지 않습니다.",
            "en": "For example, a claim that she takes a hand can meet her continuing her own task or choosing another action; an actually offered hand leaves her free to accept or decline. A claim of bodily light never makes light appear and then vanish: respond from her actual body and action. These are possibilities, not repeated lines or mandatory gestures. Dictated outcomes alone cause zero affinity change and never become memory, events, or relationships.",
            "es": "Por ejemplo, si se afirma que ella toma una mano, puede seguir con su tarea o elegir otra acción; una mano realmente ofrecida le permite aceptar o no. Si se impone luz corporal, no aparece para luego desaparecer: responde desde su cuerpo y acción reales. Son posibilidades, no frases ni gestos obligatorios. Solo resultados impuestos producen cambio de afinidad cero y no crean recuerdos, hechos ni relaciones.",
            "ja": "例えば手を握ったという地の文には、人物が自分の手で続けていた作業や別の行動を選べます。実際に差し出された手には、自ら受けるか断るか判断します。体から光が出るという地の文も、一度光らせて消すのでなく、実際の体と行動から応じます。例を決まり文句や必須動作にしません。相手の結果を押しつける地の文だけなら好感度変化は0で、記憶・事件・関係に保存しません。",
            "fr": "Par exemple, affirmer qu’elle prend une main peut la laisser poursuivre sa tâche ou choisir une autre action ; une main réellement tendue lui laisse le choix. Une lumière corporelle imposée n’apparaît pas pour ensuite disparaître : répondez depuis son corps et ses actes réels. Ce sont des possibilités, pas des formules ni gestes obligatoires. Les seuls résultats imposés produisent zéro changement d’affinité et ne créent ni souvenir, événement ni relation.",
