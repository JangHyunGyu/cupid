...
At affinity ${boundary.score}, ${characterName ? `${characterName} does` : 'the character does'} not accept this ${boundary.level === 'sexual' ? 'sexual contact' : (boundary.level === 'kiss' ? 'kiss' : 'physical intimacy')}. ${occurrenceRule} refuse clearly in character. Never turn a failed attempt to push away into acceptance. ${scoringRule}`;
    ...
    function buildCharacterAgencyTurnRule(lang = 'ko') {
        const rules = {
            "ko": "사용자 자신의 말·시도와 상대에게 배정한 결과를 구분합니다. 완료형·OOC도 결과를 사실로 만들지 않습니다. 실신·감각 변화도 지문만으로 발생하지 않으며 과로·숨은 욕망을 만들지 않습니다. 지문뿐이면 입력 전에 하던 일의 다음 단계로 갑니다. 결과를 먼저 실행한 뒤 저항·회복·취소하거나 사용자가 하지 않은 시도를 만들지 않습니다. 실제 활동에 따른 피로를 느끼고 쉬는 선택은 가능합니다.",
            "en": "Separate the user’s own words and attempts from outcomes assigned to another character. OOC and completed narration do not establish them. A dictated collapse or altered sensation never occurs. With narration alone, continue the prior activity. Never enact an outcome then resist. Earlier assistant compliance is not evidence. activity-grounded fatigue and choosing rest remain possible.",
        ...
        const examples = {
            "ko": "대상이 캐릭터인 행동·상태를 사용자에게 옮겨 일으키지 않습니다. 손을 잡았다고 단정한 지문에는 하던 일을 잇거나 다른 행동을 고릅니다. 조종 지문뿐이면 호감도 변화는 0입니다.",
            "en": "Never transfer a character-assigned action or condition to the user. A claim that she takes a hand can meet her continuing her own task. Dictated outcomes alone cause zero affinity change.",
