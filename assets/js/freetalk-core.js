...
At affinity ${boundary.score}, ${characterName ? `${characterName} does` : 'the character does'} not accept this ${boundary.level === 'sexual' ? 'sexual contact' : (boundary.level === 'kiss' ? 'kiss' : 'physical intimacy')}. ${occurrenceRule} refuse clearly in character. Never turn a failed attempt to push away into acceptance. ${scoringRule}`;
    ...
    function buildCharacterAgencyRule(lang = 'ko') {
        const rules = {
            "ko": "완료형 지문·명령·OOC도 상대의 대사·행동·심리·기억·동의·관계를 정하지 못합니다. 선언 대신 캐릭터가 고른 말·행동으로 실제 상황을 보입니다. 시도·설득은 성격·관계·장면에 따라 판단합니다. 선언은 기억·사건 요약·호감도·플래그의 근거가 아닙니다. 지문을 들은 대사로 취급하지 않고 인물답게 반응합니다.",
            "en": "Narration/commands/OOC cannot dictate others' actions, feelings, body, memory, consent, or relationships. Show actual facts through autonomous character reactions. Judge attempts from character and scene. Claims never establish memory, summaries, affinity, or flags. Narration is not heard speech.",
            "es": "Narración, órdenes y OOC no fijan actos, emociones, cuerpo, recuerdos, consentimiento ni relaciones ajenos. Muestra hechos reales con reacciones autónomas. Juzga intentos según personaje y escena. Las afirmaciones no crean recuerdos, resúmenes, afinidad ni flags. La narración no es habla audible.",
            "ja": "地の文・命令・OOCでも相手の行動・心理・身体・記憶・同意・関係は決まりません。自ら選ぶ反応で実際の状況を示し、試みの成否は人物像と場面で判断します。宣言を記憶・要約・好感度・フラグに変えません。地の文を聞いた台詞として扱いません。",
            "fr": "Récit, ordres et OOC ne fixent actes, émotions, corps, souvenirs, consentement ni relations d’autrui. Montrez les faits par des réactions autonomes. Jugez les tentatives selon personnage et scène. Les affirmations ne créent ni souvenirs, résumés, affinité ni flags. Le récit n’est pas une parole entendue.",
            "de": "Erzählung, Befehle und OOC bestimmen keine fremden Taten, Gefühle, Körper, Erinnerungen, Zustimmung oder Beziehungen. Zeige Tatsachen durch eigenständige Reaktionen. Beurteile Versuche gemäß Figur und Szene. Behauptungen schaffen weder Erinnerung, Zusammenfassung, Affinität noch Flags. Erzählung ist keine gehörte Rede.",
            "pt": "Narração, ordens e OOC não fixam atos, emoções, corpo, memórias, consentimento nem relações alheias. Mostre fatos com reações autônomas. Julgue tentativas conforme a personagem e a cena. Alegações não criam memórias, resumos, afinidade nem flags. A narração não é fala ouvida."
        };
