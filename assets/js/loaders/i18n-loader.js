...
(function() {
    ...
    async function loadDay(day) {
        ...
        if (loadedDays.has(normalizedDay)) return true;
        if (dayPromises.has(normalizedDay)) return dayPromises.get(normalizedDay);
        ...
        const promise = (async () => {
            ...
            for (const locale of locales) {
                ...
                dayLocaleData[locale] = {};
                for (let i = 0; i < slots.length; i++) {
                    const slot = slots[i];
                    const file = `day${normalizedDay}_${i+1}_${slot}.json`;
                    ...
                    const task = () => fetchJsonWithRetry(`${basePath}/${file}?v=20260926-crossing-v2`)
                        .then(data => {
            ...
            const results = await runFetchTasks(fetchTasks);
            ...
            if (failures.length > 0) {
                ...
                if (Object.keys(dayLocaleData[baseLocale] || {}).length === 0) {
            ...
            if (targetSuccessCount !== slots.length) {
                const missingTargetFiles = failures
                    ...
                    .join(', ');
                throw new Error(
                    `i18n target locale '${lang}' incomplete for day ${normalizedDay}: ${targetSuccessCount}/${slots.length}`
                    + (missingTargetFiles ? `; missing=${missingTargetFiles}` : '')
                );
            }
            const baseLocale = lang === 'ko' ? 'ko' : 'en';
            Object.assign(window.I18N_DATA, dayLocaleData[baseLocale] || {});
            if (lang !== baseLocale) Object.assign(window.I18N_DATA, dayLocaleData[lang] || {});
            loadedDays.add(normalizedDay);
            return true;
        })();

        dayPromises.set(normalizedDay, promise);
        try {
            return await promise;
        } catch (error) {
            dayPromises.delete(normalizedDay);
            throw error;
        }
    }

    window.CupidI18nLoader = {
        ensureDay: loadDay,
        prefetchDay: loadDay,
        getLoadedDays: () => Array.from(loadedDays).sort()
    };

    window._i18nReady = loadDay(1);
