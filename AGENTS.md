...
- 이벤트·엔딩 CG는 사용자가 해당 이미지에 다른 비율을 명시하지 않는 한 1:1 정사각형으로 제작한다. 최고 품질과 최대 기본 해상도를 유지한다.
- 기존 가로 CG를 정사각형으로 적용할 때는 원본을 별도 보존하고 얼굴과 주요 동작이 남도록 좌우를 자른다. 사용자 요청 없이 새로 생성하거나 축소하지 않는다. 본편과 갤러리 양쪽에서 구도를 확인한다.
