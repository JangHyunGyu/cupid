...
- `freetalk_entered`: 프리토킹 UI 초기화 완료. 특히
  ...
  호감도는 별도로 저장한다. 2.9.231부터 수락 보상은 +8이며, 개인 대화도 실제 호감도를
  올리거나 내린다. 양수는 턴당 최대 +3, 실제 점수 90 이상에서는 +2이고 저장 상한은 100이다.
  `details_json.dialogueAffinity=100`, `affinityLocked=false`로 대화 태도와 점수 정책을 구별한다.
  진입·이탈 로그의 `affinities`는 실제 점수이며, 각 응답의 증감은 기존 `chat_logs`에 남는다.
  선택 로그의 `affinityEffects`는 설정된 +8/-10 효과다. 100점 상한으로 실제 증가량은 작을 수 있다.
  이전 2.9.216 버전은 수락 +8, 전용 대화 점수 고정이었고, 2.9.219~2.9.225는 수락 +50, 2.9.226~2.9.230은 수락 +30이었다. 기존 저장에 적용한 보상은 소급 변경하지 않는다.
