...
- `freetalk_entered`: 프리토킹 UI 초기화 완료. 특히
  ...
  호감도는 별도로 저장한다. 2.9.219부터 수락 보상은 +50이며, 개인 대화도 실제 호감도를
  ...
  선택 로그의 `affinityEffects`는 설정된 +50/-10 효과다. 100점 상한으로 실제 증가량은 작을 수 있다.
  이전 2.9.216 버전은 수락 +8, 전용 대화 점수 고정이었다. 이미 적용한 보상은 소급 지급하지 않는다.
