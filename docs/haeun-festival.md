...
After the Day 4 conversation, the protagonist returns to the existing morning ending and date preparation; `day4_haeun_event_done` prevents a repeat. At the Day 5 festival, trust still below zero opens only the existing inspection follow-up: apologize and finish an inspection with Haeun (+5 Haeun trust), help without apologizing (+2), or leave (no recovery). These choices preserve the romance characters' existing scripted rewards and ending conditions. Day 5 does not start another reputation conversation.
