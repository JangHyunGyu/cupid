...
> 분기는 `assets/js/scenario/`, 문구는 `assets/js/i18n/{언어}/`에서 수정합니다. 7개 언어를 반영한 뒤 `npm run scenario:sync`로 이 문서를 갱신하고 `npm run scenario:check`로 동기화를 확인합니다.
...
- 캐릭터: `없음`
**{name}**: *갑자기 어지럽다. 속이 메슥거린다. 교문에 손을 짚고 숨을 고른 뒤에야 똑바로 선다.*
...
- 캐릭터: `없음`
**{name}**: *교문 옆에서 검은 긴 머리의 여학생이 안경을 고쳐 쓰며 시간을 확인하고 있다. 내가 가까워지자 얼굴을 든다.*
...
- 캐릭터: `dain_laugh.png`
**{name}**: *다인은 책상 위에 손가락으로 작은 코트를 그린다. 가운데 선을 긋고는 나를 향해 눈썹을 올린다.*
...
- 캐릭터: `없음`
**{name}**: *점심시간. 서연은 도시락 가방을 들고 계단으로 향하고 다인은 매점 쪽에서 손을 흔든다. 유나는 책을 챙겨 도서관으로 간다.*
...
- 캐릭터: `yuna_normal.png`
**유나**: *유나도 자기 책을 펼친다. 맞은편에 앉아 각자의 책을 읽는다.*
...
- 캐릭터: `yuna_normal.png`
**{name}**: *책을 읽다 고개를 드니 유나가 나를 보고 있다.*
...
- 캐릭터: `yuna_normal.png`
**유나**: *유나가 내가 읽던 책의 표지를 본다. 잠깐 눈이 마주친다.*
...
- 배경: `library_old.png`
- 선택지:
  1. "나머지도 읽어 보고 싶어." → `lunch_yuna_c1_1` | Yuna +4
...
- 캐릭터: `yuna_normal.png`
**유나**: *유나가 책 위에 손을 포갠다.* 조용한 거, 싫진 않지?
...
- 캐릭터: `없음`
**{name}**: *'야 소정이 새 학교에서 잘 지낸대' '다행이다 ㅋㅋ' — 익숙한 이름들이 연달아 뜬다.*
...
- 캐릭터: `없음`
**{name}**: *'다행이다 ㅋㅋ' 마지막 메시지를 다시 본다. 답장창을 눌렀지만 아무것도 쓰지 못한다.*
...
- 캐릭터: `없음`
**{name}**: *잠시 망설이다 단체방을 나갔다. 대화방 목록 맨 위에 새 학교 메신저만 남는다.*
...
- 캐릭터: `seyoun_laugh.png`
**{name}**: *서연이 클립보드 모서리를 손끝으로 맞춘다. 나는 걸음을 멈추고 대답을 고른다.*
...
- 캐릭터: `seyoun_normal.png`
**{name}**: *서연이 클립보드를 돌려 품에 안는다. 내 쪽을 보지 않고 펜 뚜껑을 닫는다.*
...
- 캐릭터: `teacher_normal.png`
**{name}**: *문밖으로 학생들의 발소리가 지나간다. 담임은 책상 위 서류를 한쪽으로 밀어 둔다.*
...
- 캐릭터: `teacher_normal.png`
**담임선생님**: *담임을 따라 교무실 문 앞 복도로 나온다. 담임은 출석부를 품에 끼고 내가 말을 이을 때까지 기다린다.*
...
- 타입: `free_talk`
- 다음: `night_start`
- 컨텍스트: "[매체: 대면] 전학 첫날 방과후 교무실 앞. 학생이 학교 적응에 관해 상담을 청했고 담임이 이야기를 들어 주었다. 그 대화를 이어가는 중이다. 학생이 먼저 찾아온 이유와 실제로 한 말을 바탕으로 편하게 묻고 답한다."
...
- 캐릭터: `nurse_normal.png`
**보건선생님**: 처음 보는 얼굴이기도 하고 교복도 새것 같아서. 맞지?
...
- 캐릭터: `nurse_normal.png`
**{name}**: *커튼 너머로 보건선생님이 기록지를 쓰는 모습이 보인다.*
...
- 캐릭터: `nurse_normal.png`
**보건선생님**: 학생 없을 땐 기록도 쓰고 물품도 챙겨. 조용하다고 늘 쉬는 건 아니야.
...
- 캐릭터: `nurse_normal.png`
**보건선생님**: *기록지를 서랍에 넣자 안쪽의 연두색 노트가 잠깐 보인다. 벽시계는 처음 누웠을 때보다 10분을 지나 있다.*
...
- 캐릭터: `nurse_normal.png`
**{name}**: *자리에서 일어나려다 멈춘다. 보건선생님은 시계를 확인하고 내 쪽으로 의자를 돌린다.*
...
- 배경: `room_school.png`
- 다음: `morning2_greet_none_2`

- 캐릭터: `없음`
**{name}**: *교실 문을 열어도 먼저 다가오는 사람은 없다. 가방 놓는 소리만 난다.*
...
- 배경: `room_school.png`
- 다음: `morning2_greet_none_neg_2`

- 캐릭터: `없음`
**{name}**: *교실 문을 열자 시선이 잠깐 모였다가 바로 흩어진다. 가방 놓는 소리만 커진다.*
...
- 캐릭터: `dain_normal.png`
**{name}**: *서연은 클립보드를 끌어안고 복도 끝으로 걸어간다.*
...
- 선택지:
  ...
  3. "따로 불러서 챙겨주시는 것도 업무예요?" → `hidden_homeroom_d2_choice2_trap` | Teacher -3
...
- 캐릭터: `teacher_normal.png`
**담임선생님**: 다른 애들도 힘든 일 있으면 찾아와. 너도 편하게 오고.
...
- 캐릭터: `teacher_normal.png`
**담임선생님**: *담임이 의자를 내 쪽으로 돌린다.* 아직 궁금한 건 없어?
...
- 캐릭터: `seyoun_normal.png`
**{name}**: 어제는 다인이랑 먹었으니까. 네 도시락 맛도 궁금했고.
...
- 캐릭터: `seyoun_normal.png`
**서연**: *서연이 도시락을 {name} 쪽으로 더 붙인다.* 앉아도 돼. 대신 우리 하던 얘기는 마저 하자.
...
- 캐릭터: `seyoun_normal.png`
**서연**: *서연은 다인이 앉은 맞은편을 보더니 자기 의자를 내 쪽으로 조금 당긴다.*
...
- 캐릭터: `seyoun_pout.png`
**서연**: *도시락 뚜껑을 열지 않은 채 난간만 본다.* 학생회 일이야. 오래 있지는 마.
...
- 캐릭터: `nurse_normal.png`
**보건선생님**: *서랍 손잡이에서 손을 뗀다.* 사적인 얘기는 이 정도만. 밴드는 괜찮지?
...
- 캐릭터: `yuna_smile.png`
**유나**: *유나는 책갈피를 끼우고 몸을 일으킨다. 복도 안쪽을 턱으로 가리킨다.*
...
- 캐릭터: `yuna_normal.png`
**유나**: 전학 간 건 나중에 들었어. 그전까지는 아무 말도 없었고.
...
- 캐릭터: `nurse_normal.png`
**{name}**: *보건선생님이 새 밴드 포장지를 버린다. 아까 급히 닫았던 서랍은 그대로다.*
...
- 캐릭터: `nurse_normal.png`
**보건선생님**: *서랍으로 향한 내 시선을 보고 손잡이에 손을 얹는다.* 그 얘기는 여기까지. 더 궁금해도 안 열어줄 거야.
...
- 캐릭터: `nurse_shy.png`
**보건선생님**: *닫힌 서랍을 검지로 두드린다.* 코코아까지 알려줬으면 충분하지. 다음 비밀은 없어.
...
- 캐릭터: `dain_active.png`
**다인**: 좋아. 서브 연습은 여기까지! 이번엔 랠리로 세 점 먼저 따기다!
...
- 캐릭터: `dain_angry.png`
**다인**: *네트 줄을 감으며 이쪽을 보지 않는다.* 오늘은 혼자 칠게. 방해만 하지 마.
...
- 캐릭터: `yuna_bored.png`
**{name}**: *별관 쪽 유나는 고개를 돌린 채 책을 든다. 눈이 마주치지 않는다.*
...
- 캐릭터: `없음`
**다인과 동행**: *다인을 따라 반납할 공 수레를 가지러 체육관으로 돌아온다. 다른 일행은 창고 담당자가 먼저 행사 자료를 옮겨 달랬다고 한다. 다인이 수레를 잡는다.* “난 공부터 가져오라고 들었는데. 어느 쪽부터 확인할래?”
...
### `after2_seo_neg_1`
- 배경: `student_room.png`
- 다음: `after2_seo_neg_leave`

- 캐릭터: `seyoun_pout.png`
**서연**: *출석 확인만 하고 펜을 내려놓는다.* 필요한 칸만 채워. 다른 얘기는 내일.
...
### `after2_seo_rival_dain_neg`
- 배경: `school_hallway.png`
- 다음: `after2_seo_rival_yuna_check`

- 캐릭터: `dain_normal.png`
**다인**: *공 바구니를 밀고 체육관으로 들어간다. 손 흔드는 시늉도 없다.*
...
### `after2_seo_rival_yuna_neg`
- 배경: `school_hallway.png`
- 다음: `after2_seo_return`

- 캐릭터: `yuna_bored.png`
**{name}**: *계단참의 유나는 책을 든 채 벽을 본다. 고개도 숙이지 않는다.*
...
- 캐릭터: `없음`
**서연과 동행**: *학생회실로 돌아와 남은 정리 시간을 확인한다. 서연이 겹쳐 적힌 일정 두 칸을 손끝으로 짚는다. 맞은편의 시선도 쉽게 물러서지 않는다.* "둘 다 네 도움이 필요해. 먼저 누구 쪽부터 갈 건데?"
...
- 선택지:
  ...
  3. "여긴 사람이 별로 없어서 좋다. 누구랑 와도 편하겠어." → `after2_yuna_trap` | Yuna -3
...
- 캐릭터: `yuna_smile.png`
**{name}**: *책을 덮을 시간이 됐다. 나는 옆에 둔 가방을 끌어당기고 유나는 소파에 놓인 물건들을 챙긴다.*
...
- 캐릭터: `yuna_bored.png`
**유나**: *책을 덮지 않는다. 쪽지도 없다.* 오늘은 같이 있기 싫어.
...
- 캐릭터: `seyoun_normal.png`
**서연**: *출석부를 팔 안쪽에 붙인 채 지나간다. 발걸음은 멈추지 않는다.*
...
- 캐릭터: `없음`
**유나와 동행**: *별관으로 돌아와 정리할 자료를 펼친다. 정리할 자료는 두 묶음인데 남은 시간은 한쪽을 끝내기에도 빠듯하다. 유나가 이어폰 한쪽을 빼고 맞은편을 바라본다.* "하나만 같이 끝낼 수 있으면, 누구 옆에 남을 건데?"
...
- 분기:
  - [`chose_yuna_lunch`] → `night2_msg_generic_3_established`
  - [기본] → `night2_msg_generic_3_first`

<!-- i18n -->

---

### `night2_msg_generic_3_established`
- 배경: `room_my.png`
- 다음: `night2_msg_7`
...
### `night2_reply_dain_1_first`
- 배경: `room_my.png`
- 스탯: Dain +4
- 다음: `night2_reply_dain_2`

- 캐릭터: `dain_normal.png` @ 0.35
**{name}**: 내일 매점에서 떡볶이 먹을까? 매운 건 잘 못 먹는데ㅋㅋ
...
- 컨텍스트: "[매체: 휴대폰 메시지] 전학 이틀째 밤, 다인과 하루를 돌아보는 대화다. 오늘 함께 있었던 장소나 활동은 저장된 사건이 있을 때만 회상한다. 방금 주고받은 메시지는 그대로 이어가며 두 사람은 아직 연인이 아니다."
...
### `night2_reply_yuna_1`
- 분기:
  - [`chose_yuna_lunch`] → `night2_reply_yuna_1_established`
  - [기본] → `night2_reply_yuna_1_first`

<!-- i18n -->

---

### `night2_reply_yuna_1_established`
- 배경: `room_my.png`
- 스탯: Yuna +4
...
### `night2_reply_yuna_1_first`
- 배경: `room_my.png`
- 스탯: Yuna +4
- 다음: `night2_reply_yuna_2`

- 캐릭터: `yuna_normal.png` @ 0.35
**{name}**: 내일 별관에 가면 책 하나 추천해줘.
...
### `night2_reply_yuna_4_first`
- 배경: `room_my.png`
- 다음: `night2_reply_yuna_5`

- 캐릭터: `yuna_normal.png` @ 0.35
**유나**: 재생목록 하나 보낼게. 세 번째 곡부터 들어봐.
...
- 캐릭터: `yuna_normal.png` @ 0.35
**{name}**: *유나가 보낸 재생목록을 연다. 세 번째 곡 옆에서 재생 버튼이 깜빡인다.*
...
- 타입: `free_talk`
- 다음: `night2_hidden_check`
- 컨텍스트: "[매체: 휴대폰 메시지] 전학 이틀째 밤, 유나와 조용히 대화를 이어간다. 책 대출, 별관 방문, 곡 공유는 저장된 사건과 방금 나눈 메시지에서 확인되는 범위만 기억한다. 두 사람은 아직 연인이 아니며 같은 장소에 있지 않다."
...
- 캐릭터: `없음`
**{name}**: *메시지에 답하려다 손을 멈춘다. 누구도 실망시키고 싶지 않다는 생각은 전 학교에서도 했었다.*
...
- 캐릭터: `없음`
**{name}**: *사진 위의 삭제 버튼에 손가락을 올렸다가 화면을 끈다.*
...
- 캐릭터: `없음`
**{name}**: *밥과 계란말이, 김에 소시지 몇 개를 더 넣고 뚜껑을 닫는다. 완성한 도시락은 냉장고에 넣어 둔다.*
...
### `morning3_seo_gate_neg_1`
- 배경: `school.png`
- 다음: `morning3_seo_gate_neg_2`
...
- 캐릭터: `yuna_shy.png`
**{name}**: *유나와 나란히 걷는다. 접은 쪽지가 손안에서 바스락거리고 두 사람의 발소리가 이어진다.*
...
- 캐릭터: `yuna_normal.png`
**유나**: *나눈 빵을 먹으며 교문을 지난다. 유나는 빈 봉지를 작게 접는다.*
...
- 캐릭터: `yuna_bored.png`
**{name}**: *유나는 눈이 마주치기 전에 고개를 돌리고 교문을 통과한다. 쪽지는 없다.*
...
### `morning3_dain_gate_neg_1`
- 배경: `school.png`
- 다음: `morning3_dain_gate_neg_2`

- 캐릭터: `dain_angry.png`
**다인**: *친구들 사이로 뛰어가며 이쪽은 보지 않는다.*
...
### `morning3_gate_none_1`
- 배경: `school.png`
- 다음: `morning3_classroom_1`

- 캐릭터: `없음`
**{name}**: *교문 앞을 지나도 먼저 서 있는 사람은 없다. 지각 종이 먼저 울린다.*
...
- 캐릭터: `teacher_normal.png`
**담임선생님**: 마지막 결과를 기다리던 날, 연락이 안 왔어. 그 무렵부터 교사가 될 준비를 했지.
...
- 캐릭터: `teacher_normal.png`
**담임선생님**: 그동안은 혼자 고쳤는데 이번엔 감상을 직접 듣겠네.
...
- 캐릭터: `teacher_smile.png`
**{name}**: *담임이 원고지를 모아 봉투에 넣는다. 마지막 장을 넣기 전에 잠깐 손을 멈춘다.*
...
- 캐릭터: `teacher_normal.png`
**담임선생님**: *원고를 파일에 넣고 교탁 옆에 선다.* 놀랐지? 잠깐은 얘기해도 되겠네. 원고 감상은 공개 합평 때 듣고.
...
- 캐릭터: `nurse_normal.png`
**{name}**: *다음 쉬는 시간에 보건실로 향한다. 팔꿈치에 붙인 밴드 끝이 소매에 걸린다.*
...
- 캐릭터: `nurse_normal.png`
**{name}**: 누군가를 실망시킬까 봐, 대답을 자꾸 미루게 돼요.
...
- 캐릭터: `nurse_normal.png`
**{name}**: 선생님도 다 챙기려다가 감당이 안 된 적 있어요?
...
- 캐릭터: `nurse_normal.png`
**보건선생님**: 대학병원에 있을 때는 호출벨이 한꺼번에 울리면 어디부터 가야 할지부터 생각했어. 환자 말을 끝까지 못 듣고 뛰어가는 날도 많았고.
...
- 캐릭터: `nurse_normal.png`
**보건선생님**: 너도 할 수 있는 것부터 말해. 다 괜찮다고 약속한 뒤 혼자 버티지는 말고. 어려우면 도와 달라고 해도 돼.
...
- 캐릭터: `nurse_normal.png`
**보건선생님**: *보건선생님이 서랍을 닫는다. 나는 문가에 서서 말을 고른다.*
...
- 캐릭터: `nurse_normal.png`
**보건선생님**: 수업까지 조금 남았네. 더 묻고 싶은 게 있으면 지금 말해도 돼.
...
- 배경: `school_hallway.png`
- 선택지:
  1. "좋아. 몇 시에 볼까?" → `morning3_date_seo_accept` | Seoyeon +4 | 플래그: `accepted_seoyeon_date`
...
### `morning3_date_seo_neg`
- 배경: `school_hallway.png`
- 플래그: `day3_seoyeon_date_withdrawn`
- 다음: `morning3_date_dain_1`

- 캐릭터: `seyoun_pout.png`
**서연**: *클립보드를 가슴 앞에 붙인 채 지나간다.* 주말 일정, 단체방에만 올릴게. 따로 묻지 마.
...
- 캐릭터: `dain_normal.png`
**다인**: *다인이 책상 모서리를 두드린다. 열린 교실 문 너머로 서류를 정리하는 서연이 보인다.*
...
- 캐릭터: `dain_laugh.png`
**{name}**: *다인과 주말 약속을 잡았다. 다인이 웃자, 문밖에 있던 서연이 클립보드를 덮는다.*
...
- 캐릭터: `dain_normal.png`
**다인**: *다인은 이쪽을 보다가 가방 지퍼를 당긴다.* 이번 주말에는 혼자 연습할 거야.

---

### `morning3_date_dain_neg`
- 배경: `room_school.png`
- 플래그: `day3_dain_date_withdrawn`
- 다음: `morning3_date_yuna_1`

- 캐릭터: `dain_angry.png`
**다인**: *가방 지퍼를 끝까지 올린다.* 주말엔 부르지 마. 혼자 연습할 거야.
...
- 캐릭터: `yuna_shy.png`
**{name}**: *쪽지 뒷면에 답을 적어 돌려준다. 유나는 내용을 읽고 접은 종이를 책 사이에 넣는다.*
...
### `morning3_date_yuna_neg`
- 배경: `room_school.png`
- 플래그: `day3_yuna_date_withdrawn`
- 다음: `morning3_check_multiple`

- 캐릭터: `yuna_bored.png`
**유나**: *쪽지를 꺼내다 다시 책 사이에 넣는다.* 주말엔 찾지 마.
...
- 캐릭터: `없음`
**{name}**: *주말 일정에 약속이 겹쳐 있다. 누구에게 먼저 이야기할지 정하지 못한 채 캘린더를 닫는다.*
...
- 캐릭터: `yuna_normal.png`
**{name}**: *점심시간. 도시락을 펼치자 서연이 옆자리에, 다인이 맞은편에 앉는다. 유나도 빵을 들고 빈 의자를 끌어온다.*
...
- 캐릭터: `yuna_bored.png`
**{name}**: *마지막 계란말이 위에서 젓가락 세 쌍이 겹칠 뻔한다. 셋이 동시에 손을 멈추고 나를 본다.*
...
- 캐릭터: `dain_sad.png`
**{name}**: *다인은 자리에서 일어나 빈 물병을 챙긴다. 서연에게 건넨 반찬을 한 번 돌아보고 복도로 나간다.*
...
- 캐릭터: `yuna_bored.png`
**{name}**: *유나는 빵 봉지를 접어 가방에 넣고 먼저 일어난다.*
...
- 캐릭터: `dain_shy.png`
**{name}**: *다인이 반찬을 다 먹고 양팔을 조금 벌린다. "고마워서 그러는데 안아도 돼?" 고개를 끄덕이자 짧게 안겼다가 물병을 집는다.*
...
- 캐릭터: `yuna_sad.png`
**{name}**: *유나는 다인의 웃음을 잠깐 바라본다. 아무 말 없이 책을 도시락 옆으로 옮기고 시선을 내린다.*
...
- 캐릭터: `yuna_shy.png`
**{name}**: *유나가 식판 옆에서 작은 쪽지를 집어 건넨다. 받아 드는 순간 두 손이 종이 끝을 함께 잡는다.*
...
- 캐릭터: `yuna_shy.png`
**{name}**: *유나가 종이 끝에서 손을 뗀다. 쪽지를 접어 조심히 챙긴다.*
...
- 캐릭터: `seyoun_normal.png`
**서연**: 내 앞에서 주고 그렇게 눈치 보면 내가 뭐가 돼. 편하게 먹어.
...
- 캐릭터: `dain_pout.png`
**다인**: 누구랑 언제 만나는지 서로 다른 얘기를 들은 것 같은데.
...
- 캐릭터: `yuna_sad.png`
**유나**: 물어본 건 알았어. 네가 여러 약속을 받아준 건 몰랐고.
...
- 캐릭터: `dain_sad.png`
**{name}**: *다인은 짧게 대답하고 시선을 피한다. 아까의 들뜬 기색은 사라졌다.*
...
- 캐릭터: `dain_normal.png`
**다인**: *공 하나만 들고 체육관 쪽으로 간다. 뒤는 보지 않는다.*
...
- 캐릭터: `없음`
**{name}**: *휴대폰을 확인한다. 주말 약속 이야기를 꺼내려면 오늘은 누구를 먼저 찾아가야 할까.*
...
- 캐릭터: `없음`
**{name}**: *주말 약속을 이대로 둘 수는 없다. 먼저 누구에게 말을 걸까.*
...
- 캐릭터: `없음`
**{name}**: *오늘은 더 말을 붙이지 못한다. 휴대폰을 가방에 넣고 교문 쪽으로 걷는다.*
...
- 캐릭터: `seyoun_normal.png`
**서연**: 일정 바뀐 거지. *클립보드를 넘기며 지나간다.*

---

### `after3_rival_seo_neg`
- 배경: `school_hallway.png`
- 다음: `after3_rival_seo_done`

- 캐릭터: `seyoun_pout.png`
**서연**: *출석부만 팔에 붙인 채 학생회실로 들어간다. 이름은 부르지 않는다.*
...
- 캐릭터: `없음`
**{name}**: *서연과는 일 얘기만 했고 다인에겐 인사만 건넸다. 하려던 말은 아직 꺼내지 못했다.*
...
### `after3_rival_dain_cool`
- 배경: `school_hallway.png`
- 다음: `after3_rival_dain_done`

- 캐릭터: `dain_normal.png`
**다인**: 오늘 체육관은 빼. *공 가방 끈을 고쳐 멘다.*

---

### `after3_rival_dain_neg`
- 배경: `school_hallway.png`
- 다음: `after3_rival_dain_done`
...
**{name}**: *도서관을 지나자 유나의 자리에 쪽지 하나만 남아 있다. "오늘은 먼저 갈게." 약속을 정리할 말은 미처 꺼내지 못했다.*
...
- 캐릭터: `seyoun_sad.png`
**서연**: *서연은 책상 밑에서 클립보드를 꺼내 무릎 위에 올린다. 입은 굳게 다문 채다.*
...
- 선택지:
  ...
  3. "네 약한 모습까지 전부 이해해줄게" → `after3_seo_overcare_react` | Seoyeon -14
  4. "이제부터는 내가 옆에서 다 챙겨줄게" → `after3_seo_overcare_react` | Seoyeon -16
...
- 캐릭터: `seyoun_normal.png`
**서연**: 그 말로 넘기지는 말아 줘. 지금은 쉽게 괜찮아지지 않으니까.
...
### `after3_seo_overcare_react`
- 배경: `student_room.png`
- 다음: `after3_seo_pity_trap_7`

- 캐릭터: `seyoun_sad.png`
**서연**: 다 이해하고 챙겨주겠다니. 내가 지금 부탁한 건 평소처럼 대해달라는 거였어.
...
- 캐릭터: `seyoun_pout.png`
**{name}**: *서연이 안경을 고쳐 쓰는 동안 펜 이야기를 이어 간다. 먼저 눈을 맞출 때까지 기다린다.*
...
- 타입: `free_talk`
- 다음: `after3_seo_end`
- 컨텍스트: "전학 사흘째 방과후 학생회실에서 마주 앉아 있다. 주인공은 울던 서연을 캐묻지 않고 펜을 줍는 척하며 평소처럼 농담했고 서연은 고맙다는 뜻으로 다음 주 일정표를 건넸다. 이 장면에서는 신뢰가 깊어졌다. 별도의 약속 갈등은 실제 플래그와 기억에 있을 때만 다룬다."
- 성격: "서연은 체면을 지켜 준 배려에 안도했다. 평소의 짧고 건조한 농담을 되찾으며 먼저 자리를 내준다. 울었던 이유를 곧바로 전부 털어놓지는 않는다."

- 캐릭터: `seyoun_normal.png`
**서연**: *서연이 클립보드를 책상에 내려놓고 옆 의자를 당긴다.* 조금 더 있다 가. 급한 일 없으면.
...
- 선택지:
  ...
  3. "걱정 마. 이 노트는 내가 대신 잘 보관할게" → `after3_yuna_control_react` | Yuna -15
  4. "이런 글은 앞으로 나한테만 보여줘" → `after3_yuna_control_react` | Yuna -18

- 캐릭터: `yuna_shy.png`
**{name}**: *유나가 빼앗아 간 노트에서 손을 거둔다.*
...
- 캐릭터: `yuna_sad.png`
**유나**: *유나는 가방끈을 어깨에 걸고 문으로 향한다. 지퍼 손잡이가 걸을 때마다 흔들린다.*
...
- 캐릭터: `없음`
**{name}**: *복도의 발소리가 멀어진다. 소파에는 노트가 눌러 놓은 자국만 남았다. 더 읽지 않겠다는 말이 너무 늦었다.*
...
- 성격: "유나는 자기 글을 처음 맡긴 상대가 신경 쓰인다. 부끄러우면 작가를 남인 것처럼 말하기도 하지만 방금 내준 신뢰를 이유 없이 거두지 않는다. 짧은 질문과 조용한 농담으로 대화를 잇는다."

- 캐릭터: `yuna_normal.png`
**유나**: *유나는 내 손에 든 노트를 흘끗 보고 소파 옆자리를 비운다.* 지금은 읽지 말고. 조금만 얘기하다 가.
...
- 선택지:
  ...
  3. "네가 무리하지 않게 내가 대신 결정해줄게" → `after3_dain_control_react` | Dain -15
  4. "걱정 마. 이제부터 내가 계속 지켜볼게" → `after3_dain_control_react` | Dain -17
...
### `after3_dain_control_react`
- 배경: `gym.png`
- 다음: `after3_dain_pity_trap_4`

- 캐릭터: `dain_sad.png`
**다인**: 내 대신 정하지 마. 지켜봐 달라고 한 것도 아니야.
...
- 다음: `after3_dain_departed_end`

- 캐릭터: `없음`
**{name}**: *체육관 문이 조용히 닫힌다. 바닥에는 다인이 놓고 간 손목 보호대가 남아 있다.*
...
- 캐릭터: `dain_angry.png`
**다인**: *다인이 신발끈을 묶으려 몸을 숙이다 무릎을 움켜쥔다. 손은 끈에 닿지 못한다.*
...
- 배경: `gym.png`
- 플래그: `dain_day3_event`, `dain_trust_deep`
...
- 컨텍스트: "전학 사흘째 방과후 체육관 벤치. 다인은 오른쪽 무릎이 계속 아픈 것을 인정했고 주인공은 대신 결정하지 않고 다인이 직접 말할 때까지 기다리기로 했다. 다인은 보건실에 갈 때 함께 있어 달라고 부탁했다. 아직 진료를 받거나 대회 출전을 결정한 것은 아니다. 둘은 앉아서 대면으로 이야기한다."
...
- 캐릭터: `seyoun_sad.png`
**서연**: 점심때 말이야. 여러 약속을 잡아놓고 왜 아무 설명도 안 했어?
...
- 캐릭터: `seyoun_cry.png`
**서연**: 미안하다는 말보다, 이제 그 약속들을 어떻게 할 건지 듣고 싶어.
...
- 캐릭터: `seyoun_pout.png`
**서연**: *학생회실 열쇠를 주머니에 넣는다.* 오늘은 여기까지. 주말 이야기도 없어.
...
- 캐릭터: `dain_angry.png`
**다인**: *네트 줄을 감고 가방을 멘다.* 오늘은 빼. 부르지 마.
...
### `minsu_d3_4`
- 분기:
  - [`chose_seoyeon_lunch && chose_seo_after2`] → `minsu_d3_4_first`
  - [`chose_dain_lunch && chose_dain_after2`] → `minsu_d3_4_first`
  - [`chose_yuna_lunch && chose_yuna_after2`] → `minsu_d3_4_first`
  - [기본] → `minsu_d3_4_established`
...
- 캐릭터: `없음`
**{name}**: *교문으로 가는 복도 끝에 누군가 기다리고 있다.*
...
- 캐릭터: `haeun_firm.png`
**하은**: 혼자 있고 싶다고 하면 기다려 주세요. 대신 집에 잘 갔는지만 나중에 물어봐 줄래요?
...
- 컨텍스트: "하은은 서연이 우는 모습을 봤다고 알렸다. 혼자 있고 싶다는 뜻은 존중하되 나중에 귀가했는지 확인해 달라고 부탁했다. 둘은 아직 복도에 서 있고 하은은 주인공의 생각을 듣고 싶다. 서연에게 즉시 돌아가거나 거절을 무시하라고 요구하지 않는다."
...
- 캐릭터: `haeun_worried.png`
**하은**: 그래도 방금 한 말은 지켜 주세요. 집에 잘 갔는지만 나중에 확인해 주시고요.
...
- 배경: `room_my.png`
- 분기:
  - [`seo_trust_deep`] → `night3_faithful_msg_seo_1`
  - [`yuna_trust_deep`] → `night3_faithful_msg_yuna_1`
  - [`dain_trust_deep`] → `night3_faithful_msg_dain_1`
  - [기본] → `night3_dream_1`
...
- 캐릭터: `yuna_normal.png` @ 0.35
**유나**: *유나에게서 짧은 메시지가 온다. 빌려준 노트 이야기다.*
...
- 캐릭터: `없음`
**{name}**: *불을 끄려다 핸드폰을 한 번 더 든다. 오늘 나란히 걷던 발소리가 생각난다.*
...
- 캐릭터: `없음`
**{name}**: *휴대폰을 내려놓는다. 내일은 무슨 말을 해야 할지 아직 모르겠다.*
...
- 캐릭터: `없음`
**{name}**: *잠들기 전 핸드폰 갤러리를 연다. 소정이 사진까지 스크롤한 뒤 길게 누른다.*
...
- 캐릭터: `없음`
**{name}**: *잠깐 화면을 내려다본다. 소정이 책상의 낙서와 고개를 돌렸던 아침들이 떠오른다.*
...
- 캐릭터: `없음`
**{name}**: *확인 창에서 취소를 누른다. 사진 한 장을 없앤다고 내가 했던 선택이 바뀌지는 않는다.*
...
- 캐릭터: `없음`
**{name}**: *사진 옆에 짧은 메모를 남긴다. 소정이에게 할 말은 새 대화방에서도 대신할 수 없다.*
...
- 캐릭터: `없음`
**{name}**: *새 폴더를 열어 사진이 그대로 있는지 확인한다. 폴더 이름 아래에 사진 한 장이 표시된다.*
...
- 캐릭터: `없음`
**{name}**: *갤러리를 닫고 내일 알람을 확인한다. 휴대폰을 내려놓은 뒤에도 잠은 바로 오지 않는다.*
...
- 캐릭터: `없음`
**{name}**: *다인의 입력창에 ‘기다리게 해서 미안해. 내일 말할게.’라고 쓴다. 엄지가 전송 아이콘 위에서 멈춘다.*
...
- 캐릭터: `없음`
**{name}**: *민수에게서 '미룬다고 없어지는 일 아님'이라는 메시지가 와 있다.*
...
- 캐릭터: `minsu_normal.png` @ 0.35
**민수**: *교실에서 주말 얘기 그렇게 하더니 모를 줄 알았냐ㅋㅋ*
...
- 캐릭터: `minsu_normal.png` @ 0.35
**{name}**: *캘린더에는 겹쳐 잡은 약속들이 남아 있다. 시간을 옮긴다고 끝날 일이 아니라는 건 이미 들었다.*
...
**다인**: 오늘이다! 체육관 앞 10시. 아, 왜 이렇게 떨리지. 이거 보내도 되나.
...
- 캐릭터: `teacher_normal.png`
**담임선생님**: 저기 안쪽 테이블에 문예부 애들 있지? 일단 여기 앉자.
...
- 캐릭터: `teacher_normal.png`
**담임선생님**: 오늘 나온 의견은 모아서 다음 시간에 같이 볼 거야.
...
- 캐릭터: `nurse_worried.png`
**보건선생님**: 나한테 먼저 말해도 돼. 다만 내가 자리를 비울 때 도와줄 사람도 있어야 하니까.
...
- 캐릭터: `nurse_normal.png`
**보건선생님**: 처음엔 같이 찾아보자. 이 안내문에 연락할 곳부터 표시해둘게.
...
- 캐릭터: `nurse_normal.png`
**보건선생님**: 그렇지? 아까는 좀 굳어 있었나 봐. 자, 이 상자까지만 마무리하자.
...
- 캐릭터: `nurse_normal.png`
**보건선생님**: 혼자 할 수야 있지. 그래도 남은 일까지 같이 끝내 줬으면 해서 물어봤어.
...
- 캐릭터: `yuna_sad.png`
**유나**: *유나가 하은에게서 눈을 떼고 나를 본다.* 하은한테 얘기는 들었어. 네 얘기도 듣고 싶어.
...
- 캐릭터: `없음`
**{name}**: *이야기가 끝나자 하은과 교문 옆으로 비켜 선다. 횡단보도 앞에는 신호를 기다리는 사람들이 모여 있다.*
...
- 캐릭터: `seyoun_laugh.png`
**서연**: 오늘은 계획 바뀌어도 괜찮아. 네가 가고 싶은 데부터 가자.
...
- 캐릭터: `seyoun_normal.png`
**서연**: 물을 자주 주지 않아도 돼. 처음 키우기엔 괜찮을 거야.
...
- 캐릭터: `dain_laugh.png`
**{name}**: *체육관 앞. 캡모자에 운동복 차림인 다인이 손을 흔든다. 오른쪽 무릎에는 보호대가 보인다.*
...
- 캐릭터: `없음`
**{name}**: *전날 써 둔 사과가 입력창에 남아 있다. 보내려는 순간 서연에게 메시지가 온다.*
...
- 캐릭터: `seyoun_normal.png`
**{name}**: *서연을 따라 학교 옥상으로 올라왔다. 난간 아래로 운동장 조명이 켜진다.*
...
- 배경톤: `warm`
- 선택지:
  1. "좋아한다고 말한다." → `confess_seo_yes_1` | 플래그: `confessed_seoyeon`
...
- 배경톤: `lights-off`
- 선택지:
  1. "좋아한다고 말한다." → `confess_dain_yes_1` | 플래그: `confessed_dain`
  2. "벤치에 조금 더 앉아 있을까?" → `confess_dain_no_1` | 플래그: `postponed_dain`
...
- 캐릭터: `없음`
**{name}**: *학교에서 집으로 향할 시간이다. 소매에 묻은 먼지를 털고 휴대폰을 확인한다.*
...
### `day4_nurse_checkin`
- 배경: `school.png`
- 다음: `day4_adult_nurse_student_rank`
...
- 캐릭터: `seyoun_sad.png`
**서연**: *교문으로 향하다 옥상 계단 앞의 서연을 만난다. 서연은 소매 끝을 쥐고 말을 고른다.* 오늘은 나랑 조금 더 있으면 안 돼?
...
- 캐릭터: `dain_sad.png`
**다인**: *교문으로 가는 길에 체육관 앞의 다인이 손짓한다. 벤치에 앉아 무릎 보호대를 고쳐 매고 있다.* 바로 가야 해? 오늘은 뛰는 거 말고 나랑 얘기 좀 하자.
...
- 캐릭터: `yuna_sad.png` @ 0.35
**유나**: *별관 앞을 지날 때 유나에게서 사진이 온다. 책상 옆 의자 하나가 비어 있다.* 아직 학교면, 잠깐 올래? 오늘은 네가 조금 더 있었으면 해.

---

### `day4_teacher_checkin`
- 배경: `school.png`
- 다음: `day4_adult_teacher_student_rank`
...
- 캐릭터: `없음`
**서연**: *옥상 문이 닫히자 서연은 {name}의 손목을 잡아 자기 가슴 위에 손을 얹는다. 느슨하게 푼 리본 아래로 셔츠 깃이 벌어지고 떨리는 숨이 손등에 닿는다. 주머니 속 휴대폰이 연달아 울리지만 서연은 손을 놓지 않는다.* 내 심장, 뛰는 거 느껴져?
...
- 캐릭터: `없음`
**다인**: *마지막 공이 바닥을 굴러간다. 다인은 거친 숨을 고르며 유니폼 앞자락을 움켜쥐고 아래로 당긴다. 그대로 {name} 바로 앞까지 몸을 깊이 숙이자 땀에 젖은 숨이 코앞에 닿는다. 주머니 속 휴대폰이 연달아 울린다.* 유니폼 안으로 손 넣어 봐. 가슴 사이 땀, 손가락으로 느껴 보고 싶지 않아?
...
- 캐릭터: `없음`
**{name}**: *집에 도착해 학교 앱의 귀가 확인을 마친다. 현관문을 닫고 나서야 신발을 벗는다.*
...
- 캐릭터: `없음`
**유나**: *유나는 {name} 바로 앞까지 다가와 한 손으로 책장을 짚는다. 재킷을 한쪽 어깨 아래로 흘려 내리고 다른 손으로 안쪽 상의의 목깃까지 끌어내린다. 드러난 맨어깨와 타투 사이로 붉은 눈이 {name}의 눈높이에 그대로 있다. 책상 위 휴대폰이 다시 울린다.* 오늘은 천 한 장도 사이에 두고 싶지 않아. 하루 종일, 너랑만.
...
- 캐릭터: `dain_sad.png`
**다인**: 오늘은 연습 안 할 거야. 그냥… 잠깐만 안아주라. 혼자 괜찮은 척하기가 힘들어.
...
- 캐릭터: `없음`
**다인**: *마지막 공이 바닥을 굴러간다. 다인은 거친 숨을 고르며 유니폼 앞자락을 움켜쥐고 아래로 당긴다. 그대로 {name} 바로 앞까지 몸을 깊이 숙이자 땀에 젖은 숨이 코앞에 닿는다. 주머니 속 휴대폰에서는 서연의 메시지가 연달아 울린다.* 유니폼 안으로 손 넣어 봐. 가슴 사이 땀, 손가락으로 느껴 보고 싶지 않아?
...
- 선택지:
  ...
  3. "울어도 돼. 내가 다 해결해줄게" → `wall_seo_overpromise_react` | Seoyeon -4
  4. "이제부터 네가 힘들 일은 내가 알아서 막을게" → `wall_seo_overpromise_react` | Seoyeon -5
...
- 캐릭터: `seyoun_cry.png`
**{name}**: *서연을 안자 어깨에 얼굴을 묻는다. 옷감 위로 눈물이 번진다.*
...
### `wall_seo_overpromise_react`
- 배경: `park.png`
- 다음: `wall_seo_line_choice`

- 캐릭터: `seyoun_cry.png`
**서연**: *서연이 고개를 젓는다.* 전부 해결하겠다는 약속은 하지 마. 그냥 지금 내 말 좀 들어줘.
...
- 캐릭터: `없음`
**유나**: *유나는 {name} 바로 앞까지 다가와 한 손으로 책장을 짚는다. 재킷을 한쪽 어깨 아래로 흘려 내리고 다른 손으로 안쪽 상의의 목깃까지 끌어내린다. 드러난 맨어깨와 타투 사이로 붉은 눈이 {name}의 눈높이에 그대로 있다. 서연의 '공원 벤치에 있어.'라는 메시지가 화면 위에서 사라지지 않는다.* 오늘은 천 한 장도 사이에 두고 싶지 않아. 하루 종일, 너랑만.

---

### `wall_seo_skip`
- 배경: `room_my.png`
- 다음: `day4_night_reflect`

- 캐릭터: `없음`
**{name}**: *서연의 대화방은 조용하다. 전화는 오지 않는다.*
...
- 캐릭터: `없음`
**유나**: *유나는 {name} 바로 앞까지 다가와 한 손으로 책장을 짚는다. 재킷을 한쪽 어깨 아래로 흘려 내리고 다른 손으로 안쪽 상의의 목깃까지 끌어내린다. 드러난 맨어깨와 타투 사이로 붉은 눈이 {name}의 눈높이에 그대로 있다. 다인의 '체육관 뒷문 열어놨어.' 알림이 화면 위에서 사라지지 않는다.* 오늘은 천 한 장도 사이에 두고 싶지 않아. 하루 종일, 너랑만.
...
- 컨텍스트: "4일차 밤, 주인공이 유나의 제안을 수락해 별관 독서실에 둘만 남았다. 바로 전 유나가 한 손으로 책장을 짚고 재킷을 흘린 뒤 상의 목깃을 끌어내려 맨어깨와 가슴 쪽을 드러낸 채 바로 앞에 서서 천 한 장도 사이에 두고 싶지 않다고 했다. 그 거리는 아직 그대로다. 이 장면에서 유나는 말은 짧게 두고 거리와 옷을 먼저 지운다. 지금 주고받는 말과 행동에 따라 호감이 더 깊어지거나 상할 수 있다."
- 성격: "말은 짧게. 먼저 옷을 벗는다. 또 혼자 남을까 봐, 다른 쪽에 뺏기기 전에 더 가까이 안긴다. 그 접촉을 부정하지 않는다."

- 캐릭터: `없음`
**유나**: *목깃을 내린 손을 거두지 않는다.* 오늘은 천 한 장도 사이에 두고 싶지 않아.
...
- 캐릭터: `dain_sad.png`
**다인**: 오늘도 그랬어. 데이트하면서 걷는 것도 아팠는데 네 앞에서는 계속 괜찮은 척했어.
...
- 캐릭터: `없음`
**서연**: *옥상문이 닫히자 서연은 말없이 블라우스 단추를 풀고 {name}의 손목을 잡아 제 쪽으로 끌어당긴다. 열린 옷깃 사이로 손바닥이 맨가슴에 닿는 순간, 서연의 손이 그 위를 덮는다. 다인의 '체육관 뒷문 열어놨어.'라는 메시지가 화면을 밝힌다.* 내 심장, 뛰는 거 느껴져?
...
- 캐릭터: `없음`
**{name}**: *다인의 마지막 메시지는 오전에 멈췄다. 밤 알림은 없다.*
...
- 캐릭터: `없음`
**서연**: *옥상문이 닫히자 서연은 말없이 블라우스 단추를 풀고 {name}의 손목을 잡아 제 쪽으로 끌어당긴다. 열린 옷깃 사이로 손바닥이 맨가슴에 닿는 순간, 서연의 손이 그 위를 덮는다. 주머니 속에서 유나의 메시지가 한 번, 다시 한 번 울린다.* 내 심장, 뛰는 거 느껴져?
...
- 타입: `free_talk`
- 다음: `day4_temptation_return_home`
- 컨텍스트: "4일차 밤, 주인공이 서연의 제안을 수락해 옥상에 둘만 남았다. 바로 전 서연이 주인공의 손목을 잡아 자기 맨가슴 위에 손을 얹고 그 위를 덮으며 심장이 뛰는지 물었다. 그 손은 아직 그 자리에 있다. 이 장면에서 서연은 가슴 위 손을 덮은 채 낮은 목소리로 확인한다. 상대가 빼지 않으면 그 손을 더 이끈다. 지금 주고받는 말과 행동에 따라 호감이 더 깊어지거나 상할 수 있다."
- 성격: "차분한 말투로 가슴 위 손을 덮은 채 확인한다. 상대가 빼지 않으면 그 손을 더 이끈다. 다른 약속에 뺏길까 봐 손을 놓지 않는다. 큰 소리로 요구하거나 다른 사람이 되지 않는다. 그 접촉을 부정하지 않는다."

- 캐릭터: `없음`
**서연**: *잡은 손을 가슴 위에서 풀지 않는다.* 내 심장, 뛰는 거 느껴져?
...
- 캐릭터: `yuna_normal.png`
**유나**: *별관으로 들어가 독서실 안쪽 자료 보관실로 향한다. 유나가 먼지 쌓인 서가 틈에서 오래된 봉투 하나를 꺼낸다.*
...
- 캐릭터: `yuna_normal.png`
**유나**: 어느 날 전학 갔어. 직접 인사도 없이, 이 편지만 남기고.
...
- 캐릭터: `yuna_shy.png`
**{name}**: *자료 보관실 서가 사이. 유나가 내 등 뒤 한 걸음 거리에 선다.*
...
- 캐릭터: `yuna_sad.png`
**유나**: *유나는 돌아서다 문 앞에서 멈춘다. 주머니에서 떨어진 책갈피를 줍고도 한동안 손잡이를 잡지 않는다.*
...
- 타입: `free_talk`
- 다음: `day4_student_return_home`
- 컨텍스트: "4일차 밤, 별관 독서실. 유나는 이준호 선배의 편지를 읽고 지금의 주인공을 떠난 선배의 대체로 붙잡고 있는 것은 아닌지 두려움을 털어놓았다. 두 사람이 방금 한 선택과 고백의 수락·보류 상태를 유지한다. 유나 자신이 누군가의 대체라는 이야기로 뒤집지 않는다. 편지는 접어 책에 넣었고 둘은 아직 함께 있다."
- 성격: "유나는 조용하고 생각이 깊지만 이제 중요한 질문을 피하지 않는다. 비유 뒤에 숨기보다 필요한 순간에는 직접 말하며 상대의 대답을 자기 두려움에 맞춰 왜곡하지 않고 끝까지 듣는다."

- 캐릭터: `yuna_normal.png`
**유나**: *독서실 자리로 돌아온 유나가 편지를 접어 책 사이에 넣는다. 이번에는 먼저 눈을 맞춘다.* 아직 듣고 싶은 말이 있어.
...
- 캐릭터: `없음`
**다인**: *마지막 공이 바닥을 굴러간다. 다인은 거친 숨을 고르며 유니폼 앞자락을 움켜쥐고 아래로 당긴다. 그대로 {name} 바로 앞까지 몸을 깊이 숙이자 땀에 젖은 숨이 코앞에 닿는다. 유나의 이름이 휴대폰 화면에 연달아 뜬다.* 유니폼 안으로 손 넣어 봐. 가슴 사이 땀, 손가락으로 느껴 보고 싶지 않아?
...
- 타입: `free_talk`
- 다음: `day4_temptation_return_home`
- 컨텍스트: "4일차 밤, 주인공이 다인의 제안을 수락해 체육관에 둘만 남았다. 바로 전 다인이 유니폼 앞자락을 아래로 당긴 채 바로 앞까지 몸을 숙이며 유니폼 안으로 손을 넣어 가슴 사이 땀을 느껴 보라고 했다. 그 거리는 아직 그대로다. 이 장면에서 다인은 장난처럼 손을 유니폼 안으로 넣게 한다. 반응이 오면 더 대놓고 간다. 지금 주고받는 말과 행동에 따라 호감이 더 깊어지거나 상할 수 있다."
- 성격: "장난처럼 솔직하게 손을 유니폼 안으로 넣게 한다. 가슴 사이에 다른 것도 넣어보고 싶지 않냐고 물어보며 유혹한다. 반응이 오면 더 대놓고 간다. 다른 쪽 약속에 뺏기기 싫다는 말은 숨기지 않는다. 아픈 무릎을 무시하지 않는다. 그 접촉을 부정하지 않는다."

- 캐릭터: `없음`
**다인**: *유니폼 앞자락을 잡은 채 숙인 거리를 유지한다.* 유니폼 안으로 손 넣어 봐. 가슴 사이 땀, 느껴 보고 싶지 않아?
...
- 캐릭터: `없음`
**{name}**: *유나에게서는 쪽지도, 메시지도 없다. 화면만 꺼진다.*
...
- 캐릭터: `dain_normal.png`
**다인**: *체육관 앞 벤치에 나란히 앉는다. 다인은 무릎을 편하게 놓고 공을 옆에 내려둔다.* 오늘은 그냥 같이 있어 줘.
...
- 캐릭터: `dain_normal.png`
**다인**: *체육관으로 들어가자 다인이 공을 내려놓고 벤치에 앉는다. 내 쪽을 보며 옆자리를 두드린다.* 오늘은 그냥 같이 앉아 있어 줘.
...
- 캐릭터: `dain_normal.png` @ 0.35
**다인**: 아직 기다리고 있어. 못 오는 거면 말이라도 해 줘.
...
- 캐릭터: `teacher_normal.png` @ 0.35
**{name}**: *밤 9시에 올라온 공지 아래로 문예부 학생들의 확인 댓글이 달려 있다. 내일 일정에 '합평'을 적어 둔다.*
...
- 캐릭터: `teacher_sad.png`
**담임선생님**: *아침 교실. 담임이 학교 앱의 미확인 표시를 보여 준다. 휴대폰에는 어젯밤 친구와 나눈 대화가 남아 있다.* 바로 귀가한다고 했지. 일정이 바뀌었는데 왜 확인도, 연락도 없었니?
...
- 캐릭터: `nurse_worried.png`
**보건선생님**: *보건선생님이 학교 앱의 미확인 표시를 짚는다. 휴대폰에는 어젯밤 친구와 나눈 대화가 남아 있다.* 집에 간다더니 연락이 없어서 걱정했어. 어디에 있는지만 알려 줬어도 됐는데.
...
**두 사람**: *행사 안내를 앞둔 교실에서 어젯밤 일에 관련된 두 사람과 마주한다. 연락이 끊긴 이유와 서로에게 한 말을 짚는 동안 누구도 먼저 자리를 뜨지 않는다.*
"이제 어떻게 할 생각이야?"
...
- 캐릭터: `없음`
**{name}**: *전학 온 첫 주가 끝나간다. 교실 문을 열기 전, 지난 나흘 동안 오간 말들을 떠올린다.*
...
- 캐릭터: `없음`
**{name}**: 첫 주가 끝나가는데도 마음은 정하지 못했다. 일단 교문을 넘는다.
...
- 캐릭터: `없음`
**{name}**: 첫 주가 끝나간다. 잠금화면에 남은 이름 셋을 바라보다 대화방을 열지 않고 화면을 끈다.
...
- 캐릭터: `없음`
**{name}**: 첫 주가 끝나간다. 잠금화면의 이름들을 보지 않고 가방 지퍼만 올린다.
...
- 캐릭터: `없음`
**{name}**: *빈 게시판 앞에 잠시 멈춘다. 압정 자국만으로는 유나가 무슨 생각을 했는지 알 수 없다.*
...
- 캐릭터: `teacher_normal.png`
**담임선생님**: 7년째 붙잡고 있는데 이젠 다른 사람들 얘기도 좀 들어야지.
...
- 캐릭터: `nurse_normal.png`
**보건선생님**: *보건선생님이 상담실 안내 카드를 건넨다.* 긴 이야기는 상담 시간을 잡아서 하자. 몸이 아프면 보건실에는 언제든 오고. *카드를 챙겨 교실로 돌아간다.*
...
- 캐릭터: `nurse_normal.png`
**보건선생님**: *차트를 덮고 예약표를 닫는다.* 오늘은 처치만. 개인 이야기는 정식 창구로. *교실로 돌아간다.*
...
- 캐릭터: `없음`
**{name}**: *메모 앱을 연다. 소정의 일을 알리려다 만 글들, 지우고 다시 쓴 사과. 서른아홉 번째 초안은 아직 비어 있다.*
...
- 캐릭터: `없음`
**{name}**: *마흔 번째 초안은 만들지 않는다. 사과 문장을 메신저에 옮겨 적고 전송을 누른다.*
...
- 캐릭터: `없음`
**{name}**: *쪽지를 책상에 모아 두고 휴대폰 알림도 차례로 확인한다.*
...
### `day5_haeun_gate`
- 분기:
  - [`day5_haeun_event_done`] → `day5_haeun_resume`
  - [기본] → `day5_haeun_trust_check`
...
- 실시간 라이벌 분기:
  - [`Seoyeon`] → `day5_haeun_high_intro`
  - [`Yuna`] → `day5_haeun_high_intro`
  - [`Dain`] → `day5_haeun_high_intro`
  - [`Teacher`] → `day5_haeun_high_intro`
  - [`Nurse`] → `day5_haeun_high_intro`

---

### `day5_haeun_high_intro`
- 배경: `school_hallway.png`
- 다음: `day5_haeun_high_clasp`

- 캐릭터: `haeun_normal.png`
**하은**: 선배, 이쪽 좀 잡아 주세요. 방송실까지만 같이 옮겨 주실래요? *하은이 서류 상자 한쪽을 내민다.*

---

### `day5_haeun_high_clasp`
- 배경: `school_hallway.png`
- 다음: `day5_haeun_arrival_router`

- 캐릭터: `haeun_normal.png`
**{name}**: *방송실 앞에 상자를 내려놓는다. 하은의 학생회 팔찌 고리가 내 소매 단추에 걸려 있다. 하은이 고리를 풀려고 몸을 가까이 기울인다. 복도를 지나던 발걸음이 멈춘다.*

---

### `day5_haeun_arrival_router`
- 실시간 라이벌 분기:
  - [`Seoyeon`] → `day5_haeun_seoyeon_arrival`
  - [`Yuna`] → `day5_haeun_yuna_arrival`
  - [`Dain`] → `day5_haeun_dain_arrival`
  - [`Teacher`] → `day5_haeun_teacher_arrival`
  - [`Nurse`] → `day5_haeun_nurse_arrival`

---

### `day5_haeun_seoyeon_arrival`
- 배경: `school_hallway.png`
- 다음: `day5_haeun_explain_choice`

- 캐릭터: `seyoun_pout.png`
**서연**: 둘이 여기 있었네. *서연이 걸음을 멈추고 하은의 손과 내 소매를 번갈아 본다.* 뭐 하던 중이야?
...
- 캐릭터: `yuna_sad.png`
**유나**: 잠깐. *유나가 이어폰을 빼고 둘 사이를 본다.* 가까이 서 있길래.
...
- 캐릭터: `dain_angry.png`
**다인**: 야, 둘이 뭐 해? *다인이 상자 옆에 멈춰 선다.* 나 와도 되는 거지?
...
- 캐릭터: `teacher_sad.png`
**담임선생님**: *담임선생님이 서류철을 내리고 둘을 바라본다.* 여기서 뭐 하고 있었어?
...
- 캐릭터: `nurse_worried.png`
**보건선생님**: *보건선생님이 걸음을 늦춘다.* 둘이 꽤 편해 보이네. 무슨 일이야?
...
- 배경: `school_hallway.png`
- 다음: `day5_haeun_defends`

- 캐릭터: `없음`
**{name}**: *하은을 소개하고 방송실로 서류를 옮기던 중이었다고 설명한다. 하은은 고개를 끄덕이지만 상대의 시선은 아직 내 소매에 머문다.*
...
- 캐릭터: `없음`
**{name}**: *잠깐 기다려 달라고 하고 하은 쪽으로 돌아선다. 복도에 서 있는 상대에게는 나중에 설명하겠다고 덧붙인다.*
...
- 실시간 라이벌 분기:
  - [`Seoyeon`] → `day5_haeun_seoyeon_escalation_check`
  - [`Yuna`] → `day5_haeun_yuna_escalation_check`
  - [`Dain`] → `day5_haeun_dain_escalation_check`
  - [`Teacher`] → `day5_haeun_teacher_escalation_check`
  - [`Nurse`] → `day5_haeun_nurse_escalation_check`

---

### `day5_haeun_seoyeon_escalation_check`
- 분기:
  - [`day5_haeun_delayed_explanation`] → `day5_haeun_seoyeon_escalation_delay`
  - [기본] → `day5_haeun_seoyeon_escalation`

---

### `day5_haeun_seoyeon_escalation`
- 배경: `school_hallway.png`
- 플래그: `day5_haeun_misunderstanding_started`
- 스탯: Seoyeon -15
- 다음: `day5_haeun_seoyeon_group_talk`

- 캐릭터: `seyoun_pout.png`
**서연**: 편하게 부탁할 수 있는 사이라는 거네. *서연이 내 쪽으로 몸을 돌린다.* 네가 말해 봐. 둘이 언제부터 이렇게 가까웠어?
...
- 배경: `school_hallway.png`
- 타입: `group_free_talk`
- 다음: `day5_haeun_finish`

- 캐릭터: `없음`
**서연 · 하은**: *하은이 내 옆에서 반걸음 물러난다. 상대는 자리를 떠나지 않고 내 대답을 기다린다.*

---

### `day5_haeun_yuna_escalation_check`
- 분기:
  - [`day5_haeun_delayed_explanation`] → `day5_haeun_yuna_escalation_delay`
  - [기본] → `day5_haeun_yuna_escalation`
...
- 플래그: `day5_haeun_misunderstanding_started`
- 스탯: Yuna -15
- 다음: `day5_haeun_yuna_group_talk`

- 캐릭터: `yuna_sad.png`
**유나**: *유나가 이어폰을 주머니에 넣는다.* 일 도와준 건 알겠어. 그런데 둘이 가까워진 건 처음 보네. 네 얘기도 듣고 싶어.
...
- 캐릭터: `yuna_sad.png`
**유나**: *유나가 이어폰을 주머니에 넣는다.* 하은이 부탁한 건 알겠어. 그래도 네가 직접 말해 줘. 둘이 뭐 하던 거야?
...
- 캐릭터: `없음`
**유나 · 하은**: *하은이 내 옆에서 반걸음 물러난다. 상대는 자리를 떠나지 않고 내 대답을 기다린다.*

---

### `day5_haeun_dain_escalation_check`
- 분기:
  - [`day5_haeun_delayed_explanation`] → `day5_haeun_dain_escalation_delay`
  - [기본] → `day5_haeun_dain_escalation`
...
- 플래그: `day5_haeun_misunderstanding_started`
- 스탯: Dain -15
- 다음: `day5_haeun_dain_group_talk`

- 캐릭터: `dain_angry.png`
**다인**: *다인이 하은에게서 내 쪽으로 시선을 돌린다.* 하은 말고 네가 얘기해. 그냥 일만 도와주던 거 맞아?
...
- 캐릭터: `없음`
**다인 · 하은**: *하은이 내 옆에서 반걸음 물러난다. 상대는 자리를 떠나지 않고 내 대답을 기다린다.*

---

### `day5_haeun_teacher_escalation_check`
- 분기:
  - [`day5_haeun_delayed_explanation`] → `day5_haeun_teacher_escalation_delay`
  - [기본] → `day5_haeun_teacher_escalation`
...
- 플래그: `day5_haeun_misunderstanding_started`
- 스탯: Teacher -15
- 다음: `day5_haeun_teacher_group_talk`

- 캐릭터: `teacher_sad.png`
**담임선생님**: *담임선생님이 서류철을 옆구리에 끼운다.* 하은이 대신 대답할 필요는 없어. 네가 설명해 봐. 내가 잘못 본 거야?
...
- 캐릭터: `없음`
**담임선생님 · 하은**: *하은이 내 옆에서 반걸음 물러난다. 상대는 자리를 떠나지 않고 내 대답을 기다린다.*

---

### `day5_haeun_nurse_escalation_check`
- 분기:
  - [`day5_haeun_delayed_explanation`] → `day5_haeun_nurse_escalation_delay`
  - [기본] → `day5_haeun_nurse_escalation`
...
- 플래그: `day5_haeun_misunderstanding_started`
- 스탯: Nurse -15
- 다음: `day5_haeun_nurse_group_talk`

- 캐릭터: `nurse_worried.png`
**보건선생님**: *보건선생님의 웃음이 멎는다.* 하은한테 묻는 게 아니야. 너한테 듣고 싶어. 둘이 무슨 사이야?
...
- 캐릭터: `없음`
**보건선생님 · 하은**: *하은이 내 옆에서 반걸음 물러난다. 상대는 자리를 떠나지 않고 내 대답을 기다린다.*
...
- 실시간 라이벌 분기:
  - [`Seoyeon`] → `day5_haeun_concern_seoyeon_intro`
  - [`Yuna`] → `day5_haeun_concern_yuna_intro`
  - [`Dain`] → `day5_haeun_concern_dain_intro`
  - [`Teacher`] → `day5_haeun_concern_teacher_intro`
  - [`Nurse`] → `day5_haeun_concern_nurse_intro`

---

### `day5_haeun_concern_seoyeon_intro`
- 배경: `school_hallway.png`
- 다음: `day5_haeun_concern_seoyeon_arrival`

- 캐릭터: `haeun_worried.png`
**하은**: *복도 모퉁이에서 하은의 목소리가 들린다. 하은은 맞은편 사람에게 낮은 목소리로 이야기하다 나를 보고 말을 멈춘다.* 전에 선배랑 얘기할 때 좀 불편했어요. 어떻게 받아들여야 할지 몰라서 말씀드렸어요.

---

### `day5_haeun_concern_seoyeon_arrival`
- 배경: `school_hallway.png`
- 다음: `day5_haeun_concern_choice`
...
- 배경: `school_hallway.png`
- 다음: `day5_haeun_concern_yuna_arrival`
...
- 배경: `school_hallway.png`
- 다음: `day5_haeun_concern_dain_arrival`
...
- 배경: `school_hallway.png`
- 다음: `day5_haeun_concern_teacher_arrival`
...
- 배경: `school_hallway.png`
- 다음: `day5_haeun_concern_nurse_arrival`
...
- 실시간 라이벌 분기:
  - [`Seoyeon`] → `day5_haeun_concern_seoyeon_check`
  - [`Yuna`] → `day5_haeun_concern_yuna_check`
  - [`Dain`] → `day5_haeun_concern_dain_check`
  - [`Teacher`] → `day5_haeun_concern_teacher_check`
  - [`Nurse`] → `day5_haeun_concern_nurse_check`

---

### `day5_haeun_concern_seoyeon_check`
- 분기:
  - [`day5_haeun_delayed_explanation`] → `day5_haeun_concern_seoyeon_escalation_delay`
  - [기본] → `day5_haeun_concern_seoyeon_escalation`
...
- 플래그: `day5_haeun_misunderstanding_started`
- 스탯: Seoyeon -15
- 다음: `day5_haeun_concern_seoyeon_group_talk`
...
- 분기:
  - [`day5_haeun_delayed_explanation`] → `day5_haeun_concern_yuna_escalation_delay`
  - [기본] → `day5_haeun_concern_yuna_escalation`
...
- 플래그: `day5_haeun_misunderstanding_started`
- 스탯: Yuna -15
- 다음: `day5_haeun_concern_yuna_group_talk`
...
- 분기:
  - [`day5_haeun_delayed_explanation`] → `day5_haeun_concern_dain_escalation_delay`
  - [기본] → `day5_haeun_concern_dain_escalation`
...
- 플래그: `day5_haeun_misunderstanding_started`
- 스탯: Dain -15
- 다음: `day5_haeun_concern_dain_group_talk`
...
- 분기:
  - [`day5_haeun_delayed_explanation`] → `day5_haeun_concern_teacher_escalation_delay`
  - [기본] → `day5_haeun_concern_teacher_escalation`
...
- 플래그: `day5_haeun_misunderstanding_started`
- 스탯: Teacher -15
- 다음: `day5_haeun_concern_teacher_group_talk`
...
- 분기:
  - [`day5_haeun_delayed_explanation`] → `day5_haeun_concern_nurse_escalation_delay`
  - [기본] → `day5_haeun_concern_nurse_escalation`
...
- 플래그: `day5_haeun_misunderstanding_started`
- 스탯: Nurse -15
- 다음: `day5_haeun_concern_nurse_group_talk`
...
- 배경: `school_hallway.png`
- 다음: `day5_haeun_low_choice`

- 캐릭터: `haeun_firm.png`
**하은**: 점검표 찾으세요? *하은이 서류를 자기 쪽으로 당긴다.* 아직 선배한테 혼자 맡기기는 어려워요.
...
- 캐릭터: `haeun_worried.png`
**하은**: 부스 번호랑 비상 통로만 다시 확인하면 돼요. 도와주실 거면 저랑 같이 가요.

---

### `day5_haeun_low_apology`
- 배경: `school_hallway.png`
- 다음: `day5_haeun_low_work`

- 캐릭터: `haeun_firm.png`
**하은**: *하은에게 지난번 태도를 사과하고 점검을 같이 하겠다고 말한다. 하은은 잠시 기다렸다가 점검표를 편다.* 사과는 들었어요. 그럼 여기부터 같이 봐 주세요.

---

### `day5_haeun_low_work`
- 배경: `school_hallway.png`
- 플래그: `day5_haeun_helped`
- 다음: `day5_haeun_low_result`

- 캐릭터: `없음`
**{name}**: *하은과 부스 번호를 하나씩 맞춰 본다. 통로에 나온 빈 상자를 벽 쪽으로 옮기고 빠진 안내판 위치를 점검표에 적는다. 마지막 칸까지 확인한 뒤 서류를 하은에게 돌려준다.*

---

### `day5_haeun_low_result`
- 분기:
  - [`day5_haeun_apologized`] → `day5_haeun_low_repair`
  - [기본] → `day5_haeun_low_thanks`

---

### `day5_haeun_low_repair`
- 배경: `school_hallway.png`
- 플래그: `day5_haeun_event_done`, `day5_haeun_trust_repaired`
- 스탯: Haeun +5
- 다음: `day5_haeun_resume`

- 캐릭터: `haeun_relieved.png`
**하은**: *하은이 채워진 점검표를 넘겨 본다.* 끝까지 해 주셨네요. 지난번 일은 아직 걸리지만 오늘 도와주신 건 고마워요.
...
- 캐릭터: `haeun_normal.png`
**하은**: *하은이 마지막 칸에 확인 표시를 한다.* 덕분에 일찍 끝났어요. 서류는 제가 가져갈게요.
...
- 캐릭터: `haeun_firm.png`
**하은**: 알겠어요. 점검은 제가 할게요. *하은이 점검표를 접어 들고 부스 쪽으로 걸어간다.*
...
- 캐릭터: `seyoun_normal.png`
**{name}**: 처음 인사할 때, 너무 긴장해서 무슨 말을 해야 할지도 몰랐어.
...
- 캐릭터: `seyoun_laugh.png`
**서연**: 그랬어? 인사는 꼬박꼬박 하던데. 그때부터 보고 있었어.
...
- 캐릭터: `seyoun_normal.png`
**서연**: *학교를 한 바퀴 돌고 옥상으로 올라온다. 서연이 난간에 손을 얹는다.* 행사 날 점심도 결국 여기서 먹네.
...
- 캐릭터: `yuna_smile.png`
**유나**: *가방에서 새 노트를 꺼낸다. 전에 쓰던 원고를 고쳐 옮긴 흔적이 여백마다 남아 있다.*
...
- 캐릭터: `yuna_normal.png`
**유나**: *책등을 손가락으로 훑다가 고개를 든다.* 오늘은 좀 말이 많았지.
...
- 캐릭터: `dain_normal.png`
**다인**: *굴러온 배구공을 운동화 밑으로 눌러 세운다.* 행사 날 점심인데, 이렇게 끝내긴 아쉽잖아.
...
- 캐릭터: `haeun_normal.png`
**하은**: 그럼 조금 더 얘기해요. 선배가 저를 어떻게 보고 있는지 듣고 싶어요. 저도 제 마음을 천천히 얘기해 볼게요.
...
- 캐릭터: `haeun_firm.png`
**하은**: 오늘 얘기해 줘서 고마워요. 그런데 저는 아직 선배랑 사귈 마음까지는 아닌 것 같아요. *하은이 가방을 챙긴다. 인사를 나누고 학교 쪽으로 발길을 돌린다.*
...
- 캐릭터: `teacher_angry.png`
**담임선생님**: 상담은 시간을 다시 잡자. 오늘은 행사 정리부터 마칠게.
...
- 캐릭터: `nurse_angry.png`
**보건선생님**: 몸이 아프면 보건실로 와. 이 이야기는 다음 상담 때 마저 하자.
...
- 캐릭터: `yuna_smile.png`
**{name}**: *책갈피의 짧은 문장을 다시 본다. ‘재밌었어.’ 마지막 글자 아래에 잉크가 조금 번져 있다.*
...
### `after5_farewell_yuna_neg`
- 배경: `library_old.png`
- 다음: `after5_farewell_dain_check`

- 캐릭터: `yuna_bored.png`
**유나**: *책을 든 채 복도 반대쪽으로 간다. 말은 없다.*
...
- 캐릭터: `dain_angry.png`
**다인**: *가방을 고쳐 메고 체육관 문으로 들어간다. 뒤는 보지 않는다.*
...
### `after5_hidden_teacher_neg`
- 배경: `room_school.png`
- 배경톤: `empty`
- 다음: `after5_hidden_return_to_park`

- 캐릭터: `teacher_normal.png`
**담임선생님**: *원고 봉투를 서류철에 넣고 덮개를 닫는다.* 오늘은 행사만 마치자. 개인 이야기는 여기서 끝이다.
...
### `after5_hidden_nurse_neg`
- 배경: `nurse_room.png`
- 다음: `after5_hidden_return_to_park`

- 캐릭터: `nurse_normal.png`
**보건선생님**: *상담 기록지를 파일에 넣고 예약표를 닫는다.* 다음부터는 정식 창구로만. 오늘은 여기까지.
...
### `after5_last_chance_seo_neg`
- 배경: `school_back.png`
- 다음: `after5_confess_fail_walk_1`

- 캐릭터: `seyoun_pout.png`
**서연**: *출구 쪽으로 먼저 걷는다.* 오늘은 됐어.
...
### `after5_last_chance_dain_neg`
- 배경: `school_back.png`
- 다음: `after5_confess_fail_walk_1`

- 캐릭터: `dain_angry.png`
**다인**: *공 가방을 멘 채 지나간다.* 부르지 마.
...
- 캐릭터: `yuna_normal.png`
**유나**: *유나가 돌아본다. 책을 끌어안은 팔에 힘이 들어간다.* 왜.
...
- 캐릭터: `없음`
**{name}**: 좋아해. 친구 이상으로. 늦었지만 내 마음은 직접 말하고 싶었어.
...
- 캐릭터: `seyoun_normal.png`
**서연**: *공원 쪽으로 나란히 걷기 시작한다.* 늦었어. 그래도… 이제는 네 마음 알겠어.
...
- 캐릭터: `dain_laugh.png`
**다인**: *학교를 나서며 운동화 끈을 내려다본다.* 야, 너. 나 그 말 기다렸어. 다시 모른 척하기 없기야.
...
### `after5_confess_react_dain_neg`
- 배경: `school_back.png`
- 다음: `after5_confess_fail_walk_1`

- 캐릭터: `dain_angry.png`
**다인**: *공을 집어 들고 먼저 돌아선다.* 그 말, 지금 들을 생각 없어.
...
- 캐릭터: `yuna_sad.png`
**유나**: *유나가 책을 꼭 쥔다.* 들었어. 근데 아직 대답은 못 하겠어.

---

### `after5_confess_react_yuna_neg`
- 배경: `school_back.png`
- 다음: `after5_confess_fail_walk_1`

- 캐릭터: `yuna_bored.png`
**유나**: *책장만 넘긴다.* 그 말은 받지 않아.
...
- 캐릭터: `없음`
**{name}**: *메시지 목록의 세 이름을 바라본다. 각자에게 학교 뒤편에서 잠깐 만나자고 보낸다.*
...
- 캐릭터: `없음`
**{name}**: *잠시 뒤 세 사람이 차례로 온다. 휴대폰을 넣고 마주 선다. '한 사람을 고르지 못했어.' 말을 마쳐도 누구도 바로 답하지 않는다.*
...
- 캐릭터: `없음`
**{name}**: *공원 쪽으로 함께 걷는다. 한동안 아무도 말을 꺼내지 않는다.*
...
- 캐릭터: `dain_sad.png`
**다인**: 그날 명단에서 네 이름 지웠어. 아직 다시 적을 생각은 없어.
...
- 캐릭터: `teacher_sad.png`
**담임선생님**: 졸업 축하해. 서류는 빠진 것 없는지 확인하고. 잘 지내.
...
- 캐릭터: `nurse_worried.png`
**{name}**: *열린 보건실 문 앞에서 멈춘다. 선생님이 기록지를 정리하다 나를 보고 펜을 내려놓는다.*
...
- 캐릭터: `nurse_worried.png`
**보건선생님**: 상담 기록은 마무리했어. 도움이 필요하면 새로 예약해.
...
- 캐릭터: `없음`
**{name}**: *보건선생님이 빈 컵을 치우고 책상으로 돌아간다. 나는 문 앞에서 인사한 뒤 돌아선다.*
...
- 캐릭터: `없음`
**{name}**: *고백한 뒤 공원까지 함께 걸어왔다. 대답을 기다리며 신발 끝을 내려다본다.*
...
- 캐릭터: `없음`
**{name}**: *소정에게 사과를 보내기까지 미뤄 둔 말들이 떠오른다. 이번에는 말할 수 있을 때 말하고 싶다.*
...
- 캐릭터: `없음`
**{name}**: *소정에게 보낸 사과를 다시 읽는다. 그 아래에는 새 말풍선이 없다. 답장을 기다리던 화면이 어둡게 꺼진다.*
...
- 캐릭터: `없음`
**담임선생님**: 책이 나올 때까지 기다려 줘서 고마워. 그 기념으로 주는 꽃이야. 그리고… 다음에도 보고 싶다는 뜻으로 받아 줘도 될까?
...
- 캐릭터: `teacher_normal.png`
**담임선생님**: *회사 출입증을 보고 웃는다.* 일은 좀 할 만해?
...
- 캐릭터: `teacher_smile.png`
**담임선생님**: 행사는 끝났네. 조금 더 앉아 있을까, 전시를 보러 갈까?
...
- 캐릭터: `teacher_smile.png`
**담임선생님**: *북토크가 끝난 서점에서 연락처 화면을 열어 보인다.* 다음에 만나면 요즘 어떻게 지내는지 더 들려줘. 지금 조금 얘기해도 좋고.
...
- 캐릭터: `없음`
**보건선생님**: 자다 깨니 얼굴이 안 좋았어. 아침약 가져왔어.
...
- 캐릭터: `없음`
**{name}**: *이불 속에서 손가락이 다시 맞닿는다. 스탠드는 켜져 있다.*
...
- 캐릭터: `없음`
**{name}**: *선생님이 웃음을 거둔다. 침대 옆 스탠드가 꺼지고 머리 위에서 형광등 소리가 난다.*
...
- 캐릭터: `없음`
**{name}**: *불빛이 돌아오자 다른 얼굴이 보인다. 보랏빛 머리와 금빛 눈. 두 알의 약을 내밀던 손은 같은 자리에 있다.*
...
- 캐릭터: `nurse_normal.png`
**{name}**: *테이블 위 휴대폰 두 대가 울린다. 한쪽에는 야근 승인 알림이, 다른 쪽에는 건강행사 종료 보고가 떠 있다.*
...
- 캐릭터: `nurse_normal.png`
**보건선생님**: 보건실에 와서도 괜찮다는 말부터 하던 건 기억나.
...
- 캐릭터: `nurse_normal.png`
**보건선생님**: *커피잔 손잡이를 손가락으로 두드린다.* 응. 다음엔 시계 안 보고 얘기하자.
...
- 캐릭터: `nurse_normal.png`
**보건선생님**: 응. 계속 이 일을 하고 싶어서 남았어. 너를 다시 만나기로 한 것도 내가 정했고.
...
- 캐릭터: `없음`
**{name}**: 저도 다음 주에 또 보고 싶어요. *영화가 끝나기 전, 주원은 {name}의 어깨에 기대 잠든다.*
...
- 캐릭터: `없음`
**{name}**: *서연의 방은 처음인데, 책상 위에는 낯익은 물건이 많았다.*
...
- 캐릭터: `yuna_normal.png` @ 0.35
**{name}**: *교문을 나온다. 비가 내린다. 유나가 남긴 마지막 쪽지를 다시 펼친다. 종이 아래쪽이 비에 번져 있다.*
...
- 캐릭터: `없음`
**{name}**: *다인이 팔을 풀자 나도 천천히 물러선다. 두 발로 편하게 서는 걸 확인한 뒤 벤치에 나란히 앉는다.*
...
- 캐릭터: `없음`
**{name}**: *그날 다인은 '괜찮아'라는 말 대신 다음 진료 날짜를 알려 줬다.*
...
- 캐릭터: `없음`
**{name}**: *일요일 저녁, 집으로 돌아가는 길. 교문 앞을 지나며 걸음을 늦춘다. 내일도 이 문으로 들어오겠지만 오늘 나눈 말은 전과 다르다.*
...
- 캐릭터: `없음`
**{name}**: *전학 온 수요일부터 닷새. 첫 주의 마지막 칸에 내일의 약속을 적는다.*
...
- 캐릭터: `없음`
**{name}**: *다인은 내일 아침 알람 이름을 ‘재활 스트레칭’으로 바꾼다. 중계 연습도 찍어 보내겠다고 한다. 끝까지 보고 답하겠다고 약속했다.*
...
- 캐릭터: `seyoun_laugh.png`
**서연**: *공원 길에서 걸음을 늦춘다.* 이제야 말해 줬으니까. 앞으로는 그렇게 오래 미루지 마.
...
- 분기:
  - [`chose_yuna_lunch`] → `good_epilogue_1_yuna_established`
  - [기본] → `good_epilogue_1_yuna_first`
...
- 캐릭터: `없음`
**{name}**: *유나가 새 쪽지를 내민다. ‘같이 읽고 싶은 책을 골랐어. 내일 도서관에서 빌리자. — 유나’*
...
- 캐릭터: `없음`
**{name}**: *쪽지를 가방 안쪽에 넣는다. 오늘 읽지 못한 페이지는 내일도 그 자리에 있을 것이다.*
...
- 호감도 변동: `없음`
- 다음: `day5_credits`
- 컨텍스트: "유나 GOOD END 뒤의 대화. 4일차 고백 뒤 어색함이 남았지만 함께 책을 읽고 다음 날도 만나기로 했다. 1권 대출과 2권 약속은 해당 기억이 있을 때만 언급한다. 이제 시작한 관계를 이별이나 장기 연애로 바꾸지 않는다."
...
- 캐릭터: `yuna_smile.png`
**유나**: *책갈피를 끼우고 책을 덮는다.* 말해 줘서 고마워. 다음엔 나도 먼저 얘기할게.
...
- 캐릭터: `없음`
**{name}**: *다인이 먼저 주먹을 내밀고 기다린다. 내 손에 든 재활 안내지에는 다음 산책 시간과 '속도는 내가 정하기'가 적혀 있다.*
...
- 호감도 변동: `없음`
- 다음: `day5_credits`
- 컨텍스트: "다인 PERFECT END의 1년 후 대화. 다인은 재활을 이어 가며 스포츠미디어학과 중계부에서 첫 방송을 마쳤고 어머니도 재방송을 봤다. 주인공은 약속대로 방송을 끝까지 봤다. 지금은 방송 부스에서 감상을 듣고 다음 대본을 준비한다. 부상과 진로, 안정된 관계를 유지한다."
...
- 캐릭터: `dain_laugh.png`
**다인**: *주먹을 내밀고 웃는다.* 늦었지만 세이프! 이제는 말 미루기 없기다?
