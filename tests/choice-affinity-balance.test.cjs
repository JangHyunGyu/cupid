...
test('day-three incidents rejoin day-four meetings without erasing distance or setting a deferred confession', () => {
    for (const incident of ['day3_caught_multiple_dates', 'harem_seed']) {
        for (const [character, short] of [['Seoyeon', 'seo'], ['Yuna', 'yuna'], ['Dain', 'dain']]) {
            ...
            assert.equal(renderer.resolveAffinityGuard(scenes[id]), `date_${short}_skip`);
        }
    }
});
...
test('day 4 rival temptations grant +8 while retaining relationship costs and localization', () => {
    const counteroffers = [
        {
    ...
    for (const counteroffer of counteroffers) {
        ...
        for (const choice of scene.choices) {
            ...
            if (choice === scene.choices[1]) assert.equal(total, -2, `${counteroffer.sceneId} acceptance must cost more trust than it gains`);
...
test('all twelve +8 acceptances retain the incident ending even at 100 affinity', () => {
    ...
    for (const [id, scene] of accepts) {
        ...
        assert.deepEqual(Object.values(accept.stats).map(stat => stat.affinity).sort((a, b) => a - b), [-10, 8], id);
    ...
    for (const [sceneId, scene] of competitiveScenes) {
        ...
        for (const choice of scene.choices.slice(0, 2)) {
            ...
            if (choice.setFlags?.includes('day4_counteroffer_penalty_deferred')) {
                assert.ok(temptationAcceptanceSceneIds.has(sceneId), `${sceneId} is not a registered +8 temptation acceptance`);
                assert.equal(choice, scene.choices[1]);
                assert.deepEqual(deltas.sort((a, b) => a - b), [-10, 8]);
            } else {
...
test('live affinity guards enforce relationship thresholds and provide localized exits', () => {
    const guardedEntries = [
        ...
        ['night2_reply_dain_1', 'Dain', -19, 'night2_reply_dain_react_neg'],
        ['night2_reply_yuna_1', 'Yuna', -19, 'night2_reply_yuna_react_neg'],
        ['night2_msg_generic', 'Dain', -19, 'night2_msg_generic_2'],
        ['night2_msg_generic_2', 'Seoyeon', -19, 'night2_msg_generic_3'],
        ['night2_msg_generic_3', 'Yuna', -19, 'night2_msg_7'],
        ['night2_msg_seo_general', 'Seoyeon', -19, 'night2_msg_yuna_general'],
        ['night2_msg_yuna_general', 'Yuna', -19, 'night2_msg_7'],
        ['night2_msg_dain_general', 'Dain', -19, 'night2_msg_dain_general_merge'],
        ['night2_msg_seo_general_alt', 'Seoyeon', -19, 'night2_msg_seo_general_alt_gate'],
        ['morning4_seo_msg_1', 'Seoyeon', 30, 'morning4_hidden_check'],
        ['morning4_yuna_msg_1', 'Yuna', 30, 'morning4_hidden_check'],
        ['morning4_dain_msg_1', 'Dain', 30, 'morning4_hidden_check'],
        ['night3_faithful_msg_seo_1', 'Seoyeon', 20, 'night3_dream_1'],
        ['night3_faithful_msg_yuna_1', 'Yuna', 20, 'night3_dream_1'],
        ['night3_faithful_msg_dain_1', 'Dain', 20, 'night3_dream_1'],
        ...
        ['tour_yuna_1_check', 'Yuna', 40, 'tour_yuna_skip'],
        ['tour_dain_1_check', 'Dain', 40, 'tour_dain_skip'],
        ['after5_farewell_seo_1', 'Seoyeon', 40, 'after5_farewell_seo_skip'],
    ...
    for (const [sceneId, character, minAffinity, fallback] of guardedEntries) {
        ...
        for (const copy of localizedCopies) {
            if (scenes[fallback]?.routeBeforeRender) continue;
    ...
    for (const [character, short] of [['Seoyeon', 'seo'], ['Yuna', 'yuna'], ['Dain', 'dain']]) {
        ...
        for (const affinity of [-100, -1, 0, 19, 20, 29, 30, 39, 40, 100]) {
            ...
            assert.equal(renderer.resolveAffinityGuard(scenes[`morning3_date_${short}_1`]),
                affinity < 20 ? `morning3_date_${short}_skip` : null);
            for (const entry of [`after3_auto_${short}`, `after3_rivals_for_${short}_start`]) {
                assert.equal(renderer.resolveAffinityGuard(scenes[entry]),
                    affinity < 30 ? `after3_${short}_skip` : null, `${entry}/${affinity}`);
            }
            for (const chooser of ['after3_choice', 'after3_multi_choice']) {
                ...
                assert.equal(renderer.resolveAffinityGuard(scenes[choice.next]),
                    affinity < 30 ? `after3_${short}_skip` : null);
            }
            if (affinity < 30) {
        ...
        const negativeRefusal = scenes[`after3_${short}_neg_1`];
        assert.ok(negativeRefusal.clearFlags.includes(routeFlag));
        assert.ok(negativeRefusal.clearFlags.includes(promiseFlag));
    ...
    for (const locale of ['ko', 'en', 'ja', 'es', 'fr', 'de', 'pt']) {
        ...
        for (const [lead, short] of [['Seoyeon', 'seo'], ['Yuna', 'yuna'], ['Dain', 'dain']]) {
            for (const visits of [{}, { nurse_day4: true }, { homeroom_day4: true }, { nurse_day4: true, homeroom_day4: true }]) {
                ...
                let id = 'day4_night_start';
                const visited = new Set();
                while (!scenes[id]?.choices?.some(choice => choice.setFlags?.includes('day4_counteroffer_penalty_deferred'))) {
                    assert.ok(scenes[id] && !visited.has(id), `${locale}/${lead}: unexpected route at ${id}`);
                    visited.add(id);
                    const scene = renderer._applyI18n(scenes[id], id);
                    for (const flag of scene.setFlags || []) flags[flag] = true;
                    id = renderer.resolveAffinityGuard(scene) || renderer.resolveNextScene(scene);
                }
                ...
                assert.equal(accept.stats[lead].affinity, -10);
                assert.equal(accept.stats[rival].affinity, 8);
                assert.equal(accept.stats.Teacher, undefined);
                assert.equal(accept.stats.Nurse, undefined);
                assert.ok(!accept.setFlags.some(flag => /^day4_counteroffer_target_(teacher|nurse)$/.test(flag)));
...
test('adult day-four routes remain available without an established student route', () => {
    for (const [flag, next] of [['homeroom_day4', 'day4_teacher_checkin'], ['nurse_day4', 'day4_nurse_checkin']]) {
        const flags = { [flag]: true, route_seoyeon: false, route_yuna: false, route_dain: false };
        assert.equal(createSceneRenderer({}, flags).resolveNextScene(scenes.day4_night_branch), next);
    }
    assert.equal(createSceneRenderer({}, {}).resolveNextScene(scenes.day4_night_branch), 'day4_student_night_branch');
});
