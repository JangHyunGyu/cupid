...
for (const mode of ['high', 'low']) for (const [index, character] of ['Seoyeon', 'Yuna', 'Dain', 'Teacher', 'Nurse'].entries()) {
    test(`${mode}/${character}: buildup, persisted rival, entry loss, group turns and archive`, async ({ page }) => {
        ...
        const entry = await page.evaluate(async ({ character, mode, delayed }) => {
            ...
            return { seen, selected: e.stateManager.getFlag('day5_haeun_rival'), affinity: e.stateManager.getAffinity(character), scene: e.sceneRenderer.currentSceneId };
        }, { character, mode, delayed });
        expect(entry.selected).toBe(character);
        expect(entry.affinity).toBe(delayed ? 60 : 65);
        expect(entry.seen).toContain(mode === 'high' ? 'day5_haeun_high_clasp' : 'day5_haeun_concern_clarify');
        await page.reload();
        await ready();
        ...
        const sessionId = `day5_haeun_${mode === 'low' ? 'concern_' : ''}${character.toLowerCase()}_group_talk`;
        ...
        if (turns === 5) {
            ...
            expect(await page.evaluate(() => window.gameEngine.sceneRenderer.currentSceneId)).toBe(mode === 'high' ? 'day5_haeun_finish' : 'day5_haeun_low_intro');
        }
